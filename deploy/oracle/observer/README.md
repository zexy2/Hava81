...
Deploy a merged observer revision with `sudo bash deploy/oracle/install-observer.sh`. The installer runs the versioned observer tests and Python compilation first, takes the same runtime lock used by the oneshot collector while replacing files, keeps the timer enabled, runs one collection, verifies the timer is still enabled/active, and prints the resulting status. This avoids stopping the five-minute timer or exposing a partially replaced collector.

After any observer deployment, inspect `state.json` and confirm the expected new checks are present and healthy. The Nginx target check follows `/var/lib/hava81/current-api-port`, which is updated only after a validated blue/green traffic switch; if that state file is absent on a legacy install, the observer falls back to port 4002.
