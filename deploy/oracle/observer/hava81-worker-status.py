...
print(f"merge_deploy_ready: {worker.get('can_merge_or_deploy')} blockers={worker.get('merge_deploy_blocking_reasons', [])} observer_writes={worker.get('writes_repository')}")
...
api_build_recovery_gib = round(api_build_bytes_to_free / (1024 ** 3), 2) if isinstance(api_build_bytes_to_free, (int, float)) else None
print(f"host_disk: ok={disk.get('ok')} warning={disk.get('pressure_warning', False)} used_pct={disk.get('used_percent')} used_gib={used_gib} free_gib={free_gib} recovery_gib={recovery_gib} api_build_ready={disk.get('api_build_headroom_ok')} api_build_recovery_gib={api_build_recovery_gib} issues={host.get('issues', [])} warnings={host.get('warnings', [])}")
