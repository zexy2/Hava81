...
export const UVIndex: React.FC<UVIndexProps> = ({ value, className = '' }) => {
  const { t } = useTranslation();
  const hasValue = typeof value === 'number' && Number.isFinite(value);
  const safeValue = hasValue ? value : 0;
  const { level, rotation, segments } = useMemo(() => {
    let levelIndex = 0;
    if (safeValue < 3) levelIndex = 0;
    else if (safeValue < 6) levelIndex = 1;
    else if (safeValue < 8) levelIndex = 2;
    else if (safeValue < 11) levelIndex = 3;
    else levelIndex = 4;

    // Gauge rotation: map 0-12+ to 0-180 degrees
    const rot = Math.min((safeValue / 12) * 180, 180);
  ...
  if (!hasValue) {
    return (
      <div className={`uv-index uv-index--unavailable ${className}`}>
        <h4 className="uv-index__title">{t('weather.uvIndex')}</h4>
        <div className="uv-index__empty" role="status">
          <span className="uv-index__empty-value" aria-hidden="true">
            --
          </span>
          <span className="uv-index__empty-label">{t('uv.unavailable')}</span>
        </div>
      </div>
    );
  }
    ...
    <div className={`uv-index ${className}`}>
      ...
      <div className="uv-index__gauge">
        <svg viewBox="0 0 120 70" className="uv-index__svg">
          ...
          {/* Needle */}
          <g transform={`rotate(${rotation - 180}, 60, 60)`} className="uv-index__needle">
            <polygon points="60,15 57,55 60,58 63,55" fill={level.color} />
