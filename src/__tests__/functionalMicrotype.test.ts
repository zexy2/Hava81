...
describe('functional microtype readability', () => {
  ...
  it('keeps product section kickers at or above the 13px-equivalent floor', () => {
    const css = readFileSync('src/styles/App.css', 'utf8');
    expect(remFontSize(cssRule(css, '.atlas-kicker'))).toBeGreaterThanOrEqual(0.8125);
  });
  ...
  it('keeps route planning hint at or above the 13px-equivalent floor', () => {
    const css = readFileSync('src/components/hava81/RouteWeatherPanel.css', 'utf8');
    expect(remFontSize(cssRule(css, '.route-weather__hint'))).toBeGreaterThanOrEqual(0.8125);
  });
  ...
  it('keeps Forecast Atlas temperature unit at or above the 13px-equivalent floor', () => {
    const css = readFileSync('src/components/hava81/ForecastAtlas.css', 'utf8');
