...
describe('modeled alert provenance', () => {
  it('keeps safety-sensitive detached notification titles visibly tied to Hava81', () => {
    for (const title of [en.hava81.alerts.windTitle, en.hava81.alerts.airTitle, en.hava81.alerts.difficultTitle]) {
      expect(title).toContain('Hava81');
    }
    for (const title of [tr.hava81.alerts.windTitle, tr.hava81.alerts.airTitle, tr.hava81.alerts.difficultTitle]) {
      expect(title).toContain('Hava81');
    }
  });
