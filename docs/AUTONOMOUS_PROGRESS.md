...
## 2026-08-29 03:01 TRT — map label matches rendered data

- Audited the map surface against `WeatherMap`: it renders the selected city's weather marker plus province/plate shortcuts, not meteorological observation stations.
- Replaced the misleading `İstasyon haritası` / `Station map` eyebrow with `Türkiye · İl haritası` / `Türkiye · Province map`. Map tiles, weather values, city coordinates and interaction behavior are unchanged.
- Validation: lint, type-check, App integration 6/6, and `git diff --check`.
