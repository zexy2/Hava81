...
## 2026-08-28 18:12 TRT — observer freshness hardening

- During direct production verification, explicitly probed the readiness endpoint with a cache-busting request and confirmed the live API returns a request-time timestamp plus Cache-Control: no-store. The existing five-minute observer only checked HTTP/status/CORS and could not distinguish a syntactically healthy stale readiness payload from a fresh one.
- Hardened the Oracle read-only observer to capture readiness cache policy, parse the payload timestamp, require an age of at most 180 seconds, and expose both values in state.json and the compact status command. Ran the oneshot immediately: production remained HEALTHY, readiness age was 0.0s, cache policy no-store, provider circuit closed, and Nginx remained on 4002.
- Versioned the exact active observer, compact status script, hardened service and five-minute timer under deploy/oracle/observer/, with runtime-path and validation notes so the host configuration is reproducible instead of existing only as server-local state.
- Validation: both versioned Python files compile without writing bytecode, all four versioned runtime files are byte-for-byte identical to the active host copies, the built-in observer smoke suite passes 2/2 including a stale-readiness failure case, systemd-analyze verify returns success for the versioned service/timer, and the live oneshot produced a healthy fresh state snapshot.
