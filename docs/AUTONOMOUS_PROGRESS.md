...
### 2026-09-02 19:56 TRT — exact resume checkpoint after AQ freshness merges

- Current `origin/main`: `6fd3903ffcb15421f7e2f3ea6a8580c2b36f3052` (`Expire stale air-quality guidance (#689)`). Its post-merge main CI/CD run is `33657714600` (#1678) and CodeQL run is `33657714593` (#566); both were still in progress at this checkpoint.
- PR #691 was merged as `f6bea03ad2dc660f65c02f2a6cdb38e9a1f739e1` after exact-head CI/CD `33652842015` and CodeQL `33652842033` passed. Post-merge main CI/CD `33657166543` (#1675) and CodeQL `33657166532` (#563) both passed; public root, `/istanbul/`, and all six boot assets returned 200 afterward.
- PR #689 was rebased onto post-#691 main with both append-only documentation histories preserved, pushed with exact lease from `05d63e16a2d81072a576acc2b8eb8f38122b3006` to `8343fa03bbaaccf77cfa7a612fce9a15b8dc639a`, then merged only after exact-head CI/CD `33657282561` (#1676) and CodeQL `33657282460` (#564) passed and production was reverified healthy.
- PR #692 (`Fail closed on stale AQ handoff`) is open and mergeable on current main. It was rebased after #689 and pushed with exact lease from `2cde6defa1fa752fdb2566cc4ef0931db28ef955` to exact head `bf190a9798470e7d5b7610c1aaf9d6bbbdecb0ea`. `git diff --check` passes; new exact-head hosted runs had not appeared yet at the checkpoint. Do not merge until CI/CD and CodeQL for `bf190a97...` are both green and production/main is directly reverified.
- Host disk remains the active operational incident: root is ~98% used with ~1.3 GiB free. The hardened Hava81 merged-worktree cleaner safely removed three clean merged linked worktrees plus one rebuildable test-results directory, preserving branch refs, but reclaimed only ~9 MB. Docker reports only ~126 MB reclaimable images and no dangling images/build cache; active containers/volumes and production images were left untouched. Large unrelated/user storage (`.hermes`, its backup, job-search, browser profiles, Postify trees, etc.) was deliberately not touched.
- Production remained healthy at last direct/observer verification: root and `/istanbul/` 200, boot assets 200, API readiness fresh/no-store via observer, OpenWeather circuit closed, CORS correct, nginx still on 4002. The disk incident remains the reason observer `can_merge_or_deploy` may report false; use fresh direct production checks plus GitHub exact-head gates before any merge.
- Next queue: (1) watch main #1678/#566 and smoke-test post-#689 production when green; (2) require exact-head hosted gates for PR #692 `bf190a97...`, then reverify SHA/mergeability/production and merge; (3) watch its main pipeline and smoke-test; (4) continue freshness/trust-boundary audit or low-risk UX/accessibility work in a separate clean worktree; (5) continue disk remediation only with positively proven Hava81-owned rebuildable/merged candidates, never unrelated runtime or user data.
