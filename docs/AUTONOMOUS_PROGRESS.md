...
## 2026-09-05 14:17 TRT — gate the desktop map until current weather exists
- Continued independently from exact main `8c833523c72fd7468c271ea73e1582c1b8894d68` while #996/#997 validate in separate branches.
- Audited the root no-city flow and found the desktop header map action remained enabled before any current weather existed, even though the map panel is intentionally rendered only inside the weather dashboard. Activating it could therefore select a destination that could not render.
- Added the same native disabled-state contract already used by neighboring header controls: the desktop map button is disabled until current weather exists. Existing city-page map behavior is unchanged.
- Extended integration and desktop Chromium regressions to require the root map trigger disabled and no map region before weather. `git diff --check` passes locally; exact-head hosted CI/CD + CodeQL/browser/Lighthouse remain mandatory before merge.
- No map data, weather values, provider/freshness semantics, scoring, MGM guidance, API runtime, or safety guidance changed.
