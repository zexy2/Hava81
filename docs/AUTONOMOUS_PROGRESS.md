...
### 2026-09-01 20:25 TRT — Pages exact-shell verifier covers observed propagation latency
- PR #573 merged green at exact head `80601a8633467bee458b9aa1bd37368ec0168d91`; main became `aa2402510c6391b7a36d5361abf63d5692d417c9`.
- Main CI/CD #1345 passed frontend, API, production build, browser flows, Lighthouse, and Docker. The Pages publish itself succeeded (`gh-pages` commit `3063a06`), but `Verify public static shell` failed because the custom-domain root still served the previous exact HTML hash after the verifier's 12 x 5 s window.
- Direct cache-busted polling proved propagation, not a broken release: the root stayed on old sha256 `23a23c56...` through 17:22:53Z and changed to the expected `a14ac8aa...` at 17:23:04Z, about 141 seconds after the gh-pages push completed at 17:20:43Z. No rollback was warranted because the previous production release stayed healthy throughout and the new exact shell eventually arrived.
- Increased the default exact-shell retry budget from 12 to 36 attempts at the existing 5-second interval. The verifier remains finite (~175 seconds of inter-attempt delay) and still requires byte-for-byte equality for root/city shell/current JS/CSS assets; this changes only how long CI tolerates real Pages propagation.
- Validation of the retry-budget change: `bash -n` and `git diff --check` pass. To avoid local-build hash nondeterminism contaminating the test, exported the exact current `origin/gh-pages` tree (`3063a06`) to a temporary deploy fixture and ran the modified verifier with `HAVA81_SMOKE_ATTEMPTS=1`; root, İstanbul, manifest, service worker, brand assets, every current boot JS/CSS asset, and a retained prior-generation asset all matched the live custom domain on the first check after propagation completed.
