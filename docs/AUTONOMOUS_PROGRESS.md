...
### 2026-09-02 14:15 TRT — run checkpoint after #666 merge and #671 rebase
- Fresh observer at 14:13 TRT reported production healthy: root/İstanbul/readiness/boot-assets/CORS all green, OpenWeather readiness fresh with `no-store`, and nginx still on port 4002. Host remains under root-disk pressure at ~93.1% used / ~3.1 GiB free.
- PR #666 (`Hide empty forecast atlas after total failure`) completed exact-head CI/CD #1623 and CodeQL #511 successfully and was merged as main `03e6d8f78530750dcf0673e4408587e0585e73ab`. Main post-merge CI/CD #1625 (`33623502962`) and CodeQL #513 (`33623503119`) are in progress; continue independent work while they validate, then smoke normal root and `/istanbul/` shells plus boot assets when green.
- PR #671 (`Drop stale daily rows on partial forecast recovery`) was rebased twice as main advanced, preserving append-only autonomous decisions/progress. Current exact head is `e57e9376607b50e152f46998ef11b2ba1351ca13`, based directly on `03e6d8f78530750dcf0673e4408587e0585e73ab`. Exact-head CI/CD #1626 (`33623574478`) is pending and CodeQL #514 (`33623574455`) is in progress. Do not merge until both are green and head/mergeability are reverified immediately beforehand.
- Hardened merged-worktree cleanup was dry-run first, then applied: 4 clean merged linked worktrees and 3 rebuildable artifact directories were removed; branch refs were preserved and production/container/runtime data was untouched. Disk pressure remains, so continue only bounded repository-owned cleanup and do not remove Docker/browser/user data blindly.
- Resume queue: (1) re-check main #1625/#513 and production smoke after #666 propagation; (2) while pending, continue an isolated current-main UX/accessibility/performance audit; (3) when #671 exact-head gates are green, verify remote head `e57e9376607b50e152f46998ef11b2ba1351ca13`, mergeability, fresh observer and direct production health, then merge; (4) watch the resulting main pipeline and smoke production; (5) continue disk-pressure investigation using evidence-based, repository-owned candidates only.
