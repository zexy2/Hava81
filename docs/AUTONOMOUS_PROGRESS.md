...
## 2026-08-30 08:07 TRT — harden traffic-switch readiness contract

- After PR #268 merged as main `a08ac192f72298463cef2be1b2351391693c9ddc`, audited the traffic switch on isolated `automation/hava81-switch-readiness-0805`. `switch-api-traffic.sh` retried `curl -fsS` but treated any 2xx body as ready and did not verify freshness, `no-store`, or production CORS before writing current/previous API port markers.
- Added `deploy/oracle/validate-api-readiness.py` and wired both local target and post-switch public probes through it. Accepted readiness now requires `status=ready`, a timezone-aware timestamp within the same 180s freshness / 60s future-skew envelope used by the observer, and `Cache-Control: no-store`. The public probe sends the production Origin and requires exact ACAO; a failure follows the existing rollback path before state-marker updates.
- Added seven validator unit tests covering valid local/public responses plus non-ready status, stale/future timestamps, cacheable health responses, and wrong CORS. Combined local gates on current main pass: deploy/switch/rollback shell syntax, operation-lock regression, rollback-state regression, readiness validator 7/7, Python compile, and `git diff --check`.
- The candidate validator was also run read-only against live local port 4002 and the public readiness endpoint with the production Origin; both satisfy the stronger contract. No Nginx, container, or state mutation was performed.
- Next action: publish this branch independently, allow exact-head CI, then rebase after any earlier pending main movement before merge. Production remains deliberately on 4002 with 4001 retained for rollback/canary.
