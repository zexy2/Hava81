...
## 2026-08-31 05:30 TRT — enforce required hourly wind at the frontend boundary

- PR #395 merged as main `5734042648fc13842038afec9af32a8a641df08a`; exact main pipeline #956 completed successfully. Public GitHub Pages root and `/istanbul/` return 200 after deployment, with the root artifact updated at `2026-08-31T02:26:23Z`. Observer production checks remain healthy for root/İstanbul/API readiness/CORS and nginx 4002.
- Rebased `automation/hava81-run11-hourly-contract-0520` in `/home/ubuntu/hava81-auto-run11-contract-0520` onto exact current main `5734042648fc13842038afec9af32a8a641df08a`.
- The BFF DTO and OpenAPI contract require every hourly item to include numeric `windSpeed`; OpenWeather always maps upstream wind and the Open-Meteo provider rejects null/invalid required wind series before returning a payload. The frontend validator nevertheless accepted a missing `windSpeed`, weakening that trust boundary.
- Frontend hourly validation now fails closed when required wind is missing or invalid. Added missing-wind regressions for both forecast endpoints and corrected the temperature-domain fixture to satisfy the real BFF contract. No wind value is synthesized or substituted.
- Combined post-rebase gates pass: focused weather-service 124/124, full frontend 52 files / 472 tests, TypeScript, ESLint, production build with 81 city pages/service-worker stamping, production dependency audit 0 vulnerabilities, and `git diff --check`. Pre-rebase local Lighthouse measured 95 performance / 100 accessibility / 100 best-practices / 100 SEO.
- Next action: push/open this bounded contract PR from the exact post-#395 base and keep working in the separately isolated WeatherDecisionField future-condition audit while CI validates.
