...
## 2026-09-01 07:37 TRT — preserve native arrow keys on focused horizontal surfaces
- Follow-up interaction audit after making Route Weather and Daily Plan horizontal regions keyboard reachable found a cross-cutting conflict: app-level plain `ArrowLeft`/`ArrowRight` city-navigation shortcuts still fired from any non-editable focused element, so focusing a horizontal scroller and pressing an arrow could navigate the app instead of scrolling the focused region.
- Hardened `useKeyboardShortcuts`: plain horizontal-arrow app shortcuts now run only when the key event targets the document body/root, while focused controls and keyboard-reachable regions keep their native arrow behavior. Existing editable-control protection and global Escape close behavior remain unchanged.
- Added hook regressions proving a focused tabindex scroller does not trigger the app shortcut and body-level ArrowRight still does. Local gates pass: focused keyboard-shortcut 4/4, TypeScript, ESLint, full frontend 55 files / 527 tests, production build + service-worker stamp + all 81 city pages, dependency audit 0 vulnerabilities, and `git diff --check`.
- Main `8b325b2f5727df0508d162c9cd2145d21a48e2d1` pipeline #1205 is green and direct post-pipeline production smoke is healthy (root/İstanbul/readiness 200, fresh/no-store readiness, exact production CORS, OpenWeather circuit closed, nginx 4002). PR #517 remains isolated and must not be merged before this shortcut conflict is resolved/rebased.
