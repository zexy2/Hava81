...
- Prepared isolated branch `automation/hava81-run11-2127-next` from exact main `605bea4409f51579f1568a7532c478839bcb361a`: raise Fastify to `^5.12.1`, refresh its lockfile record/integrity, and resolve `process-warning` to 5.1.0 as required by the patched Fastify manifest. No API application code or proxy topology changed.
- Registry metadata was independently checked for Fastify 5.12.1 and process-warning 5.1.0; local JSON/lock consistency assertions and `git diff --check` pass. Node/npm remain unavailable locally, so hosted `npm ci`, API type/test/build and audit gates are mandatory.
