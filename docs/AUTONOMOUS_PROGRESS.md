...
### 2026-09-02 16:31 TRT — align static weather bootstrap cache guard with provider freshness
- Continued from exact post-#680 main `af45ed3100f685c4eb657dbf602537730bc243c8` in isolated branch `automation/hava81-bootstrap-freshness-1628` while PR #681 rebasing/deployment work proceeds independently.
- Audited the generated city-page early weather bootstrap and found it suppressed the initial network request using only the browser cache receipt age (five minutes). The hydrated app now correctly treats provider `meta.fetchedAt` / `freshForSeconds` as authoritative, so a locally recent cache containing provider-expired evidence could suppress the early request, only for React to reject it and fetch later.
- The generated bootstrap now requires both the existing bounded local receipt age and provider evidence freshness. Missing TTL keeps the established five-minute fallback; malformed, non-positive, over-one-day or materially future provider freshness evidence fails open to an early network refresh. No timestamp or weather value is rewritten.
- Added a desktop Chromium regression with a just-written browser cache whose provider evidence is two minutes old with a 60-second TTL; the generated bootstrap must issue exactly one current-weather request instead of suppressing it.
- `git diff --check` passes locally; host Node dependencies remain intentionally absent under disk pressure, so exact-head hosted frontend/API/build/browser/Lighthouse/CodeQL remain mandatory before merge.
