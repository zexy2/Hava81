...
## 2026-08-30 11:05 TRT — make decision sharing resilient to native-share failures

- Audited Daily Plan sharing from an isolated exact-main worktree based on `a96ed992c924987f7d6e713930ddacae2cc230e4` while main/PR CI continued. The prior flow preferred `navigator.share`, but any native share failure was swallowed by the outer catch, so browsers exposing a broken/unavailable native share target never reached the otherwise supported clipboard fallback.
- Native-share success still returns immediately and records one share event. User cancellation (`AbortError`) remains a no-op and deliberately does not copy. Non-cancellation native-share failures now fall through to the existing clipboard path, announce copied state, and record success only after the fallback completes. Clipboard failures remain harmless to the weather/day-plan UI.
- Added regressions for native-share failure -> clipboard fallback and AbortError -> no clipboard. Local gates pass: DailyPlan/share suites 13/13, frontend TypeScript, ESLint, 81-city production build/service-worker stamping, production dependency audit 0 vulnerabilities, and `git diff --check`.
