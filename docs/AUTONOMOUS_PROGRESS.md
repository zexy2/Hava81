...
## 2026-09-05 01:18 TRT — reject unsupported weather icon codes at the browser trust boundary
- Continued independently from exact main `7389d19b20b2b4dd56c26463bb2822d2ea088a88` while PR #925 exact-head CI runs.
- The browser forecast validator required daily/hourly icon fields to be non-empty strings, but did not require the normalized icon contract already enforced for saved favorites and emitted by both forecast providers. A malformed value such as `01x` could therefore survive the BFF trust boundary and be interpreted visually.
- Restricted daily/hourly forecast icon values to the normalized Hava81/OpenWeather families `01`–`04`, `09`, `10`, `11`, `13`, `50` plus explicit `d`/`n` suffixes. Added regression cases for unsupported daily and malformed hourly icons.
- This fails closed through the existing retryable invalid-forecast path; no valid forecast value, provider mapping, scoring, MGM semantics, UV/AQI guidance, or recommendation logic changes. `git diff --check` and dependency-free source assertions pass; hosted exact-head lint/type/unit/browser/build/Lighthouse/CodeQL remain mandatory before merge.
