...
## 2026-09-01 07:29 TRT — keyboard-accessible daily-plan timeline prepared
- Continued the horizontal-overflow accessibility audit on a third isolated branch while PR #516 and main #1203 run. The 12-hour Daily Plan timeline intentionally uses a horizontal card scroller, but unlike controls inside the other scrollers its timeline cards are non-interactive and the container itself was not keyboard reachable.
- Added `tabIndex=0` to the already-labeled timeline list and an inset `:focus-visible` outline so keyboard users can focus and scroll the timeline without compressing the decision cards or changing score/weather semantics.
- Regression coverage now requires the timeline list to be keyboard-focusable. Initial local gates pass: DailyPlanPanel 10/10, TypeScript, ESLint, production build + service-worker stamp + all 81 city pages, dependency audit 0 vulnerabilities, and `git diff --check`.
- PR #516 then passed exact-head CI/CD #1204 + CodeQL #92 and merged as main `8b325b2f5727df0508d162c9cd2145d21a48e2d1`. This timeline branch was rebased onto that exact main while preserving append-only route/activity checkpoints. Post-rebase gates pass: DailyPlanPanel + RouteWeatherPanel + ActivityPlanner 29/29, TypeScript, ESLint, full frontend 55 files / 525 tests, production build + service-worker stamp + all 81 city pages, dependency audit 0 vulnerabilities, and `git diff --check`. Main CI/CD #1205 is still running independently at this checkpoint.

## 2026-09-01 07:51 TRT — make partial commute time selection explicit
- Fresh observer/direct GitHub verification showed exact main `66301f28b7013f9c570aec03b19a37adbb89e0dc`; CI/CD #1208 completed successfully. Production root and canonical İstanbul are 200; cache-busted public API readiness is 200/ready/fresh with OpenWeather circuit closed, and observer confirms exact production-origin CORS plus nginx API port 4002.
- Host disk initially crossed the 92% merge hard gate because rebuildable validation artifacts/caches had accumulated. Removed only the completed hourly-bootstrap worktree's `node_modules/dist/test-results` plus npm caches/logs; production/rollback/canary containers, images and volumes were untouched. Fresh observer reports 91.2% and host healthy (warning-only).
- Independent UX/a11y audit found the commute planner treated a one-sided departure/return selection as the generic empty state. Unlike the Activity Planner's partial filter, neither time input was programmatically associated with an explicit “not applied yet” explanation.
- Added a distinct localized partial-selection message and conditionally connected both commute time inputs via `aria-describedby`. The app never infers the missing time; completing the pair removes the incomplete relationship and restores the existing calculated/forecast-unavailable flow. No weather values, scoring, matching windows, providers, MGM/UV/AQI or safety semantics changed.
- Regression coverage asserts the partial status and input relationships appear after one time is selected and disappear once the pair is complete. Gates pass on exact main base: focused CommutePlanPanel 7/7; full frontend 55 files / 526 tests; TypeScript; ESLint; production build + service-worker stamp + all 81 city pages; production dependency audit 0 vulnerabilities; `git diff --check`.
- Next action: fetch/reverify current main, preserve append-only checkpoints if it advanced, commit/push/open this bounded frontend-only PR and require exact-head Frontend/API/Build/Browser/Lighthouse/CodeQL green. While CI runs, continue an independent evidence-safety/mobile audit rather than waiting.
## 2026-09-01 07:55 TRT — expose same-city route validation at its source controls
- Continued independently from exact main `66301f28b7013f9c570aec03b19a37adbb89e0dc` while commute partial-state PR #519 validates; the pending #519 branch was not mutated.
- Route Weather already disabled “Koridoru kontrol et” and described that button when origin and destination were the same, but the two selects that create the invalid combination did not reference the visible validation status.
- Both endpoint selects now conditionally reference the existing `route-weather-same-city` status via `aria-describedby`; the relationship disappears automatically when endpoints differ. No route request logic, city identity, weather values, scores, providers, MGM/UV/AQI or safety semantics changed.
- Gates pass on this independent base: focused RouteWeatherPanel 10/10; full frontend 55 files / 525 tests; TypeScript; ESLint; production build + service-worker stamp + all 81 city pages; production dependency audit 0 vulnerabilities; `git diff --check`.
- Next action: keep this branch isolated until #519 settles. If #519 merges, rebase onto the new exact main, preserve both append-only checkpoints, rerun combined gates, then push/open with exact-head CI required.

## 2026-09-01 08:00 TRT — rebase route endpoint semantics after commute merge
- PR #519 exact head `2c8152cb5c8f40af42472f702c6f9d484b779588` completed CI/CD #1209 and CodeQL #97 successfully, remained mergeable against exact main, and squash-merged as main `eeff7aeaa34b0edcfa2f6e449d2846e470a35882`.
- Main CI/CD #1210 and CodeQL #98 completed successfully. Direct post-pipeline production smoke is healthy: root 200, canonical İstanbul 200, public API readiness 200/ready/fresh/no-store, exact production-origin CORS, OpenWeather circuit closed; active API port remains 4002.
- Rebasing the independent route-validation branch onto the new main produced only expected append-only documentation conflicts; both valid commute and route checkpoints/decisions were preserved. Combined post-rebase gates pass: RouteWeatherPanel + CommutePlanPanel 17/17, full frontend 55 files / 526 tests, TypeScript, ESLint, production build + service-worker stamp + all 81 city pages, dependency audit 0 vulnerabilities, and `git diff --check`.
- Removed only this worktree's rebuildable `node_modules/dist/test-results` after validation; root filesystem returned to about 91% used with >4 GiB free. Production/rollback/canary artifacts were untouched.
- Next action: exact-main/remote-branch lease recheck, amend this checkpoint into the prepared commit, push/open the bounded route a11y PR, and require exact-head CI/CodeQL before merge. Continue a separate evidence-safety/mobile audit while that CI runs.

## 2026-09-01 08:45 TRT — connect unavailable alert actions to permission help
- Started from exact production-green main `516e3e01dd0bb39791c009ae2cd3954858194370` in isolated worktree `/home/ubuntu/hava81-auto-run11-settings-dialog-0842`, branch `automation/hava81-run11-settings-dialog-0842`; unrelated dirty worktrees were left untouched.
...
## 2026-09-01 13:17 TRT — add Home/End navigation to city search suggestions
- Fresh observer/direct verification found PR #541 merged as main `72733fec6a6ce3f45c1d3cfb84497a4e42463474`; PR #542 had been independently rebased to exact head `d73ea733e7afd03c2951c8281c9d9dba0529118d`, passed CI/CD #1266 + CodeQL #154, remained mergeable, and was squash-merged as main `d73ef359d9155ed21b233ce8f1e76c4be8906d0d`. Main CI/CD #1267 is running independently; CodeQL #155 is green. Production was directly healthy immediately before merge: root/readiness 200, exact production CORS per observer, OpenWeather circuit closed, API port 4002.
- Continued from new exact main in isolated worktree `/home/chatgpt/hava81-auto-run11-1312`, branch `automation/hava81-run11-1312`; no pending branch was mutated.
- Accessibility audit found the city-search ARIA combobox supported ArrowUp/ArrowDown but not Home/End while its suggestion listbox was open. Added Home -> first option and End -> last option active-descendant navigation, preserving Enter/Escape and wrap-around arrow behavior.
- Added deterministic SearchBar regression coverage for both keys. Local gates pass: SearchBar 20/20; TypeScript; ESLint; full frontend 56 files / 538 tests; production build + service-worker stamp + all 81 city pages; production dependency audit 0 vulnerabilities; `git diff --check`.
- Next action: re-fetch current main and require it to still match/rebase if advanced; inspect diff, commit/push/open this bounded frontend-only PR, require exact-head CI + CodeQL, and continue an independent UX/a11y/performance audit while CI runs.
## 2026-09-01 13:25 TRT — expose saved-city comparison refresh as busy
- Continued from exact main `3f8bd5843229b79ace5f7f82b5b4b2b337853fb3` in isolated worktree `/home/chatgpt/hava81-auto-run11-1321`, branch `automation/hava81-run11-1321`, while PR #544 validates independently.
...
## 2026-09-01 13:50 TRT — align desktop comparison action with navigation semantics
- PR #546 exact head `52b9f13a45ba0001ae4b2d7034d2034023ae0f13` passed CI/CD #1277 and CodeQL #165, remained mergeable, and was merged as main `089222528b5450060ccd846a7250b534a26e2196`. Main CI/CD #1278 and CodeQL #166 then completed successfully; fresh observer at `2026-09-01T10:49:22Z` reports root/İstanbul/API readiness/CORS healthy, OpenWeather circuit closed, nginx on 4002, and no API deploy pending.
- Continued independently from that exact main in `/home/ubuntu/hava81-auto-run11-1348-next`, branch `automation/hava81-run11-1348-next`.
- Audited the desktop header comparison action. It navigates to the saved-city comparison page, but exposed `aria-pressed`, which models a toggle button rather than navigation. Replaced it with `aria-current="page"` only while the saved comparison view is active, matching the existing bottom navigation semantics.
- Regression coverage now requires no pressed state, no current state while idle, and `aria-current=page` after opening comparison. Updated the normal and forced-colors active-state selectors to follow `aria-current`, preserving the existing visual/high-contrast indication after the semantic correction.
- Local gates pass: focused App integration 18/18; full frontend 56 files / 538 tests; TypeScript; ESLint; production build + service-worker stamp + all 81 city pages; production dependency audit 0 vulnerabilities; `git diff --check`.
- Host note: dependency install/build temporarily lifted root disk usage above the observer warning threshold (85.8% used, ~6.87 GB free) but remains well above the hard free-space floor. Remove only this worktree's regenerable `node_modules`/`dist` after commit/publication.
- Next action: verify remote branch lease is empty and main is still `0892225...`, commit/push/open this bounded frontend-only PR, require exact-head CI + CodeQL, then continue an independent reliability/trust audit while it validates.

## 2026-09-01 14:00 TRT — bound browser transport cache retention
- Continued independently from exact stable main `089222528b5450060ccd846a7250b534a26e2196` in `/home/ubuntu/hava81-auto-run11-1355-next2`, branch `automation/hava81-run11-1355-next2`, while PR #547 validated and merged separately.
- Audited the browser BFF GET transport cache. Entries were freshness-checked before reuse but expired keys were never removed from the Map and the total retained key count was unbounded. Long-lived sessions with many unique route/departure or city/context URLs could therefore retain stale response objects indefinitely even though they were no longer reusable.
- Added bounded retention: successful GET cache writes prune expired/future-skewed entries, refresh insertion order for an overwritten key, and cap retained fresh responses at 128 by evicting the oldest retained key. This changes only client memory retention; it does not extend cache freshness, alter request payloads, weather values, scores, MGM/UV/AQI, provider semantics, or safety guidance.
- Added regressions proving the 129th distinct GET evicts the oldest response and that a new write removes expired entries. Local gates pass: focused httpClient 15/15; full frontend 56 files / 540 tests; TypeScript; ESLint; production build + service-worker stamp + all 81 city pages; production dependency audit 0 vulnerabilities; `git diff --check`.
- PR #547 exact replacement head `761fe2f7b286cce66a434a88a5fa76b1335a49c2` passed CI/CD #1280 and CodeQL #168, remained mergeable, and was merged as main `21166ddaacd573b5be603fc6f24b792ea95a2edd` after fresh observer production checks were healthy on API port 4002.
- Next action: keep this cache branch local until exact main `21166dd...` pipeline and production smoke are green, then rebase preserving append-only checkpoints, rerun combined gates, lease-check/push/open as a bounded frontend reliability PR. Continue an independent audit while the main pipeline runs.

## 2026-09-01 15:18 TRT — preserve browser bookmark and hard-refresh shortcuts
- Fresh observer/direct Git verification agreed on exact production-green main `4480fce311979ad4a591f7c25874d6923a774339`; CI/CD #1286 completed successfully and production root/İstanbul/readiness/CORS remained healthy on API port 4002. No open autonomous PR was pending.
- Continued from that exact main in isolated worktree `/home/ubuntu/hava81-auto-run11-1512`, branch `automation/hava81-run11-1512`; unrelated dirty/stale worktrees were left untouched.
- Keyboard audit found two undisclosed app shortcuts overrode standard browser commands: `Ctrl/Cmd+D` intercepted bookmarking to toggle a favorite, and `Ctrl/Cmd+Shift+R` intercepted hard reload to refresh weather. Both app actions already have explicit visible controls, while overriding browser commands makes recovery/bookmark behavior surprising and can block a hard refresh during stale-client incidents.
- Removed only those two conflicting hidden shortcuts; retained the documented search/settings shortcuts and existing Escape behavior. Weather fetching, favorites persistence, scores, providers, MGM/UV/AQI and safety semantics are unchanged.
- Added a regression contract ensuring shortcut construction does not claim bookmark/hard-refresh combinations while retaining search/settings shortcuts. Local Node/npm is unavailable in this gateway shell; `git diff --check` is the local gate and exact-head protected CI/CodeQL is mandatory before merge.
- Next action: exact-main/remote-lease recheck, commit/push/open the bounded UX PR, require exact-head CI + CodeQL, and continue an independent reliability/accessibility audit while CI runs.

## 2026-09-01 15:24 TRT — normalize offline navigation cache keys
- Continued independently from exact main `aefac7bd96d37a8e803aaeaef383497345cb42aa` in isolated worktree `/home/ubuntu/hava81-auto-run11-1520-pwa`, branch `automation/hava81-run11-1520-pwa`, while PR #552 and main #1288 validate separately.
- PWA audit found the versioned service worker cached each successful navigation using the full request URL. Hava81 city/root HTML does not vary by query string, so repeated share/tracking/cache-bust variants such as `/istanbul/?utm_source=...` could create duplicate offline shell entries for one page during a single deployed worker lifetime.
...
## 2026-09-01 15:38 TRT — render modeled-context fetch time in the weather-location timezone
- Continued independently from exact main `955cee9c8e681965e3db5a60e2b69a42b086783c` in isolated worktree `/home/ubuntu/hava81-auto-run11-1534-context-time`, branch `automation/hava81-run11-1534-context-time`; earlier pending/prepared branches were not mutated.
- Evidence/presentation audit found Context Signals formatted its provider `fetchedAt` directly in the browser/device timezone, unlike Activity, Daily Plan and Commute surfaces that explicitly use the weather location offset. A user outside Türkiye could therefore see a misleading clock time beside the modeled-data freshness attribution even though the underlying absolute timestamp was valid.
- ContextSignalsPanel now receives the current weather payload's validated `timezoneOffsetSeconds` and formats the absolute provider timestamp with that offset plus `timeZone: UTC`, so the displayed fetch clock is tied to the weather location rather than the viewer device. Future-timestamp suppression is still evaluated against the original absolute instant before formatting.
...
## 2026-09-01 16:38 TRT — production smoke verifies every current boot asset
- Continued independently from exact main `8309b34cd0b9cd0f7cbd1f713080edccc66f3c27` in isolated `automation/hava81-run11-1635-current-assets` while main #1317 and prepared CI branches validate separately.
...
## 2026-09-01 18:54 TRT — clear comparison busy state when the minimum disappears
- Continued independently from exact base `b8d1647c26ab84a833a571887b54d20628976e07` in `/home/ubuntu/hava81-auto-run11-1846`, branch `automation/hava81-run11-1846`, while PR #567/main deployment validated separately.
- ComparePanel set `loading=true` for a two/three-city request but its `<2 cities` effect path cleared rows/failures without clearing loading. If a favorite was removed while requests were pending, the panel correctly showed the “two cities required” state but the labeled region could remain `aria-busy=true` indefinitely.
- Clear loading when the selected comparison set falls below two, and add a suspended-request regression that removes one city mid-load and requires the region to settle to `aria-busy=false`. Requests already in flight remain ignored by the existing active guard. Weather values, score math, provider evidence, MGM/UV/AQI and safety guidance are unchanged.
- Local gates pass on the prepared base: ComparePanel 12/12, TypeScript, ESLint, production build + service-worker stamp + all 81 city pages, and `git diff --check`. Rebased onto exact main `b1718932e06acfa62f117909f191d181f644c3c2`; combined gates pass again: ComparePanel 12/12, TypeScript, ESLint, production build + service-worker stamp + all 81 city pages, and `git diff --check`.

## 2026-09-01 18:59 TRT — resync notification permission after external browser changes
- Continued independently from exact main `b1718932e06acfa62f117909f191d181f644c3c2` in `/home/ubuntu/hava81-auto-run11-1856`, branch `automation/hava81-run11-1856`, while PR #568 validates separately.
- DecisionAlertsPanel previously captured `Notification.permission` only at mount / Hava81-initiated permission requests. If the user changed the site permission in browser settings and returned to the tab, UI state could remain stale and an old granted state could keep attempting optional alert delivery.
- On document visibility changes, when the page is visible and Notifications are supported, resync `Notification.permission`. Existing permission gating then immediately reflects blocked/default/granted state; modeled-alert disclosure, quiet hours, dedupe, weather evidence and all safety semantics are unchanged.
- Added a regression that changes the mocked browser permission from default to denied while mounted, dispatches visibilitychange, and requires the blocked/disabled state. Local gates pass after correcting only the assertion wording: DecisionAlertsPanel 17/17, TypeScript, ESLint, production build + service-worker stamp + all 81 city pages, and `git diff --check`.

## 2026-09-01 19:14 TRT — invalidate localized route results when language changes
- Continued independently from exact main `292a1bca9492195a675282370bd7e4832091d148` in isolated `/home/ubuntu/hava81-auto-run11-daily-resize-1918`, branch `automation/hava81-run11-daily-resize-1918`, while PR #569 replacement CI validates separately.
- Route corridor responses contain provider-localized segment descriptions selected by the UI language at request time. Previously, changing Hava81 language could leave an already rendered route result — or a late in-flight response — carrying the old language under the newly localized interface.
- Language changes now invalidate the route request generation and clear result/error/loading state. A late response from the prior language is ignored by the existing request-id guard; users can explicitly re-run the corridor in the new language. Route scoring, weather values, departure validation, provider choice, modeled guidance and safety semantics are unchanged.
- Added a regression that starts a Turkish route request, changes the UI to English while it is pending, verifies the loading state settles, then resolves the stale Turkish response and requires it not to render. Local gates pass: RouteWeatherPanel 11/11, TypeScript, ESLint, production build + service-worker stamp + all 81 city pages, and `git diff --check`.

## 2026-09-01 19:29 TRT — preserve current-location refresh mode across language changes
- Continued independently from exact main `a8001c3786eba2eeeba0f559430ccb23371f8f71` in isolated `/home/ubuntu/hava81-auto-run11-location-source-1928`, branch `automation/hava81-run11-location-source-1928`, while the post-#570 main pipeline validates separately.
- `useWeather` intentionally refreshes provider-localized descriptions when the UI language changes without re-prompting for geolocation. However, that handoff previously cleared `locationAsync.data` and replaced it with a city-name request, so a later stale/resume refresh silently stopped following the user's current location and refreshed the old city instead.
- Added an in-memory active-source identity. Successful explicit location selection marks location mode; explicit city searches mark city mode. Language refresh still re-fetches localized provider text by city name to avoid a fresh geolocation prompt, but it no longer changes the source identity. Later stale/online refreshes therefore resume current-location acquisition when that was the user's chosen source.
- Added a regression that selects current location, changes Turkish -> English, verifies localized refresh uses the existing city without another location prompt, advances beyond freshness, and requires the visibility refresh to call current location again. Local gates pass: `useWeather` 52/52, TypeScript, ESLint, production build + service-worker stamp + all 81 city pages, and `git diff --check`.
- No weather values, provider interpretation, scores, MGM/UV/AQI semantics or safety guidance changed.

### 2026-09-01 19:53 TRT — main CI regression isolated and hardened
- Merged green PR #571 at exact head `f946d5b2ae713cb4622e1bb02008a8fc492d431f`; squash main became `d3b7d9f4b48a94d7bfd97dbbe3a31e407f659852`.
- Main CI/CD #1339 then failed only in `Frontend quality -> npm run test:ci`; API, lint, and type-check had passed and downstream deployment was correctly skipped. GitHub job logs showed the single failing assertion was `useWeather > preserves location as the refresh source across a language change`: the test advanced time immediately after observing the localized provider call begin, while that async refresh could still be loading.
- Reproduced the same failure on exact main in isolated worktree `automation/hava81-run11-mainfail-1947`. Hardened the regression test to await `result.current.isLoading === false` before advancing the clock and dispatching the later visibility refresh. This changes no product/weather behavior; it removes a test race while retaining the location-source contract.
- Validation after the fix: focused `useWeather` 52/52; full `npm run test:ci` 553/553 with coverage; ESLint; TypeScript; production build/service-worker stamp/81 city pages; `git diff --check` all pass.
- Production remained healthy in the fresh observer sample at `2026-09-01T16:45:07Z`: root/İstanbul/readiness healthy, CORS correct, OpenWeather circuit closed, nginx API target 4002. Host disk pressure remains a warning; test dependencies temporarily increased usage and will be removed after the branch is pushed.
- Next: commit/push this bounded test-race fix, open PR, require exact-head CI/CodeQL green, merge, then verify main deploy + public production and clean reproducible worktree artifacts.
- PR #572 CI/CD #1340 confirmed the `useWeather` race fix itself (52/52 passed), but exposed one additional pre-existing suite race in `App.integration`: the same-city resume event could fire after data rendered but before the initial weather loading flag settled. Added an observable `aria-busy=false` wait before simulating the later online event; this preserves the original product assertion and changes no runtime code.
### 2026-09-01 20:07 TRT — route result remains contained at 200% text resize
- Re-evaluated the old `automation/hava81-run11-route-result-resize-1907` idea from a fresh exact-main worktree instead of rebasing/mutating the stale branch (it was ~150 commits behind current main).
- Current main still kept the route-result heading and score in a non-wrapping flex row, which can force horizontal overflow or crush the score at large text sizes. Made the result header wrap, gave the text column a bounded flexible basis, and bounded the score width without decorative changes.
- Added a deterministic Playwright regression that mocks weather/route responses, opens a route result at desktop 1280, applies 200% root text sizing, and asserts the page, result header, and each header child remain horizontally contained.
- Validation: ESLint, TypeScript, production build/service-worker stamp/81 city pages, `git diff --check`, and focused Playwright `desktop-1280` route resize test all pass. Playwright browser files were installed into a temporary isolated path and will be removed after validation to avoid persistent disk pressure.
- PR #572 merged green at exact head `63b97a15c028c54ca3774f2a0c6777f27a9869de`; new main is `1e2cc4e2a75a86eb89e5cd82a2d149290d7ff66c`. Main #1343 passed frontend/API/build/browser/Lighthouse and public root/İstanbul plus every current boot JS/CSS asset returned 200 while Pages deployment finalized.
- PR #573's first CI #1342 failed only because it was cut from the pre-#572 main and therefore still contained the already-fixed `useWeather` lifecycle race; its own lint/type/API gates were clean. Rebased #573 onto exact current main `1e2cc4e...`, preserving both append-only documentation histories.
- Post-rebase combined local gates pass: full `npm run test:ci` 553/553 with coverage, ESLint, TypeScript, production build/service-worker stamp/81 city pages, focused 200% route Playwright 1/1, and diff checks.
- Operational reliability: repeated local `lint-staged` failures were traced to mutable `.git/refs` and `.git/logs/refs` paths (including stash and remote-ref directories) left root-owned by earlier root-run worktrees. Ownership of refs/reflogs only was restored to the canonical repository owner `ubuntu`; object database/content was untouched. A normal pre-commit will be used for this checkpoint to verify stash-based hooks work again.

### 2026-09-01 17:23Z — Pages propagation gate reliability follow-up

- PR #573 merged exact green head `80601a8633467bee458b9aa1bd37368ec0168d91`; main is `aa2402510c6391b7a36d5361abf63d5692d417c9`.
- Main CI #1345 passed frontend quality, API test/build, production build, browser flows, Lighthouse, Docker build, and pushed `gh-pages` commit `3063a06`. Only `Verify public static shell` failed: for 55 seconds the custom domain continued returning the previous root SHA-256 `23a23c56...` instead of expected `a14ac8aa...` despite HTTP 200.
...
### 2026-09-01 20:48 TRT — make recurring Hava81 worktree cleanup deterministic and fail-closed
- Started from exact main `29dc4a2a72e4eae671dfdd5840de1ed35eb3145c` in isolated `/home/ubuntu/hava81-auto-worktree-hygiene-run11-2045`, leaving the unrelated dirty primary checkout and all concurrent branches untouched.
- Root disk is again at the configured merge boundary (~92% used / ~4.0 GiB free). Repeated manual cleanup has been safe but expensive to re-audit, so added `scripts/cleanup-merged-worktree-artifacts.sh` as a dry-run-first host hygiene tool.
- Eligibility is fail-closed: a linked worktree must be clean and either an ancestor of `origin/main` or patch-equivalent to main according to `git cherry`; this safely recognizes squash-merged PR branches. The current/primary worktrees are always skipped. Only rebuildable validation directories are eligible by default; optional `--remove-worktrees` removes only the checkout while preserving its branch/ref. Docker/runtime resources are never touched.
- Current dry-run identifies 22 clean squash-equivalent linked checkouts (~58.7 MB) and zero dependency/build artifact directories; no deletion has been performed by the script yet. `bash -n`, dry-run execution and `git diff --check` pass.
- Applied the audited cleanup set after the dry-run: removed exactly 22 clean patch-equivalent linked checkouts plus 3 rebuildable validation directories (~3.8 MB); all branch refs were preserved. Root remained ~92% / ~4.0 GiB free because the meaningful remaining pressure is outside these safe Hava81 checkouts, dominated by unrelated active/shared Docker and Postify/tmp assets, which Hava81 automation intentionally did not delete.
- Added a hermetic shell regression that creates a temporary Git remote plus squash-equivalent, unmerged and dirty linked worktrees. It requires only the squash-equivalent clean checkout to be eligible/removed, requires its branch ref to remain, and requires unmerged/dirty worktrees to survive. Wired this small contract into Frontend quality CI. `bash -n`, the cleanup regression, dry-run and `git diff --check` pass.
- PR #578 exact-head CI exposed a defect in the first hermetic test fixture, not in the cleanup eligibility logic: the fixture created `dist/` without a `.gitignore`, making the intentionally squash-equivalent worktree appear dirty and therefore correctly fail closed. Updated the fixture to model the real repository's ignored generated artifacts, reran it locally, and it now passes while still proving dirty/unmerged worktrees are excluded. This failure improved the test rather than weakening the cleanup guard.


### 2026-09-01 20:54 TRT — keep route departure picker bounds current in long-lived tabs
- Continued independently from exact main `29dc4a2a72e4eae671dfdd5840de1ed35eb3145c` in `/home/ubuntu/hava81-auto-time-input-ergonomics-run11-2052` while PR #578 validates separately.
- Route submit validation already checks departure against the real current time, but the native `datetime-local` min/max were computed only on render. A tab left open for hours could therefore let the browser picker offer an already-past departure or a time beyond the current 18-hour horizon, only to reject it on submit.
- Store the timestamp used for picker bounds and refresh it on focus; use the existing `ROUTE_MAX_DEPARTURE_MS` constant for the max attribute so picker guidance and submit validation share one horizon. Added a deterministic regression that advances the mocked clock by 3.5 hours and requires min/max to advance before selection.
- Local gates pass using an existing exact-lockfile dependency tree without another install: RouteWeatherPanel 12/12, TypeScript, ESLint, production build/service-worker stamp/all 81 city pages, and `git diff --check`. Generated `dist` was removed after validation. No route score, weather evidence, provider call, MGM/UV/AQI or safety semantics changed.
...
### 2026-09-01 21:00 TRT — keep the hourly “now” marker truthful in long-lived tabs
- Audited ForecastAtlas from exact production branch lineage and found `currentLocationHourKey` was computed from `new Date()` only during React renders. A dashboard left visible across an hour boundary could therefore keep the prior hour marked as “Şimdi / Now” until some unrelated state change or weather refresh occurred.
...
### 2026-09-01 22:51 TRT — wake pending modeled alerts when quiet hours end
- Continued independently from exact main `34dbd5ed794a1b24994c9947be82132f042e763e` while main #1385 and rebased PR #587 validate separately.
- Decision alerts correctly suppress delivery during location-local 22:00–07:00 quiet hours, but a long-lived tab with an unchanged alert candidate had no clock dependency that would re-run delivery at 07:00. The pending modeled alert could therefore remain suppressed until an unrelated weather/UI update.
- Added one bounded timeout only while an enabled/granted candidate is inside quiet hours. It targets the location-local 07:00 boundary (+100 ms cushion) and increments a delivery revision so the existing dedupe, permission, storage and quiet-hours gates are re-evaluated without fetching or synthesizing weather data.
- Added a fake-clock regression starting at 06:59:30 İstanbul time that proves no notification is sent during quiet hours and exactly one is reconsidered after 07:00 without a weather refetch. `git diff --check` passes; protected hosted frontend/type/lint/browser/CodeQL gates remain mandatory because the host's retained node_modules trees have been intentionally cleaned to reduce disk pressure.

### 2026-09-01 23:04 TRT — keep saved commute plans aligned with the next outbound occurrence
- Continued independently from exact main `9ed940d6ef1da67c902d355a53b5d02f57483077` while rebased #589 validates separately and #588 main CI deploys.
...
### 2026-09-01 23:17 TRT — make merged-worktree cleanup survive stale root-owned artifacts
- Continued independently from exact main `e895dd588bcd964cd1ed8e1978063a7bffb9d988` while main #1395 and rebased PR #589 validate separately.
- A real host cleanup dry-run correctly selected only clean merged/patch-equivalent Hava81 worktrees, but `--apply` later aborted when an old generated `dist` tree and then a linked checkout contained root-owned unwritable directories. The eligible source/branch checks were safe, but one permission mismatch could stop the rest of the bounded cleanup batch after partial progress.
- Hardened the cleanup tool to preflight parent/directory removability before deleting artifacts or linked checkouts, report unwritable targets explicitly, skip them without mutation, and continue processing other independently eligible trees. Unexpected remove failures are also contained and counted instead of terminating the batch. Source files, branches, Git metadata, Docker/runtime resources and eligibility semantics are unchanged.
- Extended the hermetic shell regression with an eligible worktree containing an intentionally unwritable ignored build subtree. Dry-run must flag it as skipped; apply must preserve it while still removing the normal squash-equivalent worktree and preserving both branch refs. `bash -n`, the cleanup regression and `git diff --check` pass.
### 2026-09-01 23:31 TRT — fail closed on stale evidence before modeled alert delivery
- Continued independently from exact main `d2289f3496a2bce48046997b81db98d339f0f38a` while cleanup PR #591 and optional-evidence PR #592 validate on isolated branches.
- The quiet-hours wake can legitimately re-run delivery at 07:00, but a candidate computed before/during the night must not be sent if the underlying current/forecast evidence has crossed its provider freshness boundary. Wired the exact forecast metadata used by the decision hourly series into the production alert surface and gate both quiet-hours scheduling and delivery on current-weather plus forecast `fetchedAt`/`freshForSeconds`, with API-default fallbacks (5m current, 30m forecast) and a 60s future-skew ceiling.
- Added a fake-clock regression where an alert candidate is fresh at 06:59:30 but its forecast TTL expires before the 07:00 wake; no notification may be delivered. This does not refresh timestamps, synthesize weather, or turn modeled guidance into an official warning. `git diff --check` passes locally; exact-head hosted test/type/lint/build/browser/CodeQL gates remain mandatory because dependency trees are intentionally absent under disk pressure.

### 2026-09-01 23:22 TRT — drop expired optional evidence immediately on tab resume
- Continued independently from exact main `d2289f3496a2bce48046997b81db98d339f0f38a` while its production pipeline and PR #591 validate separately.
- AQI and modeled context already expired at provider TTL via a bounded timer, but browser background throttling could defer that timer after the tab returned. Added a visible-tab resync that re-runs the same freshness checks and reschedules only still-valid evidence; it does not fetch, extend TTLs, or synthesize values.
- Added a fake-clock regression that advances wall time beyond a 30-second TTL without running the scheduled timer, dispatches `visibilitychange`, and requires both AQI and modeled context to disappear immediately. Local dependency trees were intentionally absent under host disk pressure, so `git diff --check` is local and exact-head hosted test/type/lint/build/CodeQL gates remain mandatory before merge.

### 2026-09-02 00:50 TRT — secure first-offline-launch boot assets during service-worker install
- Continued independently from exact main `633809481a4d601037a2189eb436570387764690` while its production pipeline and rebased optional-evidence PR #592 validate separately.
- Audited the versioned service worker and found install secured only `/` plus optional manifest metadata. Because the worker may claim an already-loaded page after JS/CSS were fetched before service-worker control, the first later offline navigation could recover cached HTML but miss the hashed boot assets and render a blank app.
- Install now extracts only same-origin `/assets/` dependencies required by the root shell: script `src`, stylesheet links, and modulepreload links. Each required boot asset is fetched with `cache: 'no-store'`, cached in the same versioned shell, and a missing/non-2xx boot asset rejects the new worker so the prior worker/cache remains authoritative. Third-party scripts, icons and optional manifest metadata are not promoted to required boot dependencies.
- Added deterministic VM-backed regressions for successful boot precache, third-party/unrelated-link exclusion, and fail-closed missing boot assets. A dependency-free Node VM smoke reproduced root + CSS + modulepreload + module script caching and excluded the cross-origin script. `node --check` and `git diff --check` pass; hosted frontend/type/lint/build/browser/CodeQL gates remain mandatory before merge because dependency trees are intentionally absent under host disk pressure.

### 2026-09-02 01:12 TRT — fail closed when modeled alerts lack forecast freshness metadata
- Continued independently from exact main `3058532afb9998c861ef8f3f841879ceffd50177` while main deploy and comparison PR #595 validate separately.
- Audited the stale-evidence delivery gate from #593 and found `!forecastMeta || ...` treated missing forecast metadata as fresh. Production currently supplies `forecast.displayMeta ?? forecast.meta`, but a future partial/refactor path could therefore send a modeled decision alert without proving the hourly evidence age.
- `DecisionAlertsPanel` now requires an explicit `ForecastMeta | null`; `null` makes the evidence gate false, so neither quiet-hours scheduling nor notification delivery proceeds. Existing current/forecast TTL checks, 60-second future-skew ceiling, score logic, weather values and official-warning semantics are unchanged.
- Updated alert tests to carry realistic fresh metadata and added a regression requiring zero notification and zero sent marker when metadata is explicitly unavailable. `git diff --check` passes; exact-head hosted test/type/lint/build/browser/CodeQL gates remain mandatory because dependency trees are intentionally absent under host disk pressure.

### 2026-09-02 01:05 TRT — stop city comparison from inventing a winner on tied scores
- Continued independently from exact main `3058532afb9998c861ef8f3f841879ceffd50177` while its production pipeline validates separately.
- Audited the question-led city comparison and found its reducer kept the first favorite whenever two rows had equal top Hava81 scores. That made favorite ordering look like a weather-based decision and could label one city as the sole winner despite equal evidence.
- Comparison now derives all top-score leaders. Exactly one leader keeps the existing winner treatment; two or more leaders render an explicit localized tie and all tied city cards receive the leading visual treatment. Daily-plan scoring, weather/provider inputs, AQI/activity semantics and fetch behavior are unchanged.
- Added a deterministic regression with equal city weather inputs that requires the explicit Turkish tie copy, both city names, no single-winner label, and leading treatment on both cards. `git diff --check` passes; exact-head hosted frontend/type/lint/build/browser/CodeQL gates remain mandatory because Hava81 dependency trees are intentionally absent under host disk pressure.

### 2026-09-02 01:20 TRT — hide commute recommendations when forecast evidence expires
- Continued independently from exact main `3058532afb9998c861ef8f3f841879ceffd50177` while modeled-alert freshness and city-comparison branches validated separately; main has since advanced to `5e85556aef0d434dc6ae07a0b20af7e314b20e8b` and this branch will be rebased before push.
- Audited `CommutePlanPanel` and found that a long-lived tab could keep a previously calculated departure–return recommendation visible after the forecast metadata TTL expired. Wired the exact displayed forecast metadata from `useForecast` into the panel and gate plan construction on that evidence being fresh.
- Added one bounded timeout to the provider expiry boundary plus visible-tab resync so browser timer throttling cannot leave stale modeled guidance visible. Missing/invalid/stale metadata fails closed with explicit localized copy; no timestamp refresh, weather synthesis, implicit fetch, score change, or official-warning semantics are introduced.
- Updated commute-panel regressions to supply realistic metadata and added a fake-clock expiry test that requires an actionable recommendation to disappear after a 30-second TTL. `git diff --check` passes. Host-local Node/npm are unavailable, so exact-head hosted lint/type/unit/build/browser/CodeQL gates remain mandatory before merge.

### 2026-09-02 01:24 TRT — expire stale activity recommendations
- Continued in an isolated worktree from exact main `1da2901448d21ba2f7a886d2fb691e5424951c66` while rebased PRs #597 and #598 validate independently.
- Audited `ActivityPlanner` and found its score/best-window cards could remain visible indefinitely in a long-lived tab even after the hourly forecast metadata TTL elapsed. Wired the exact displayed forecast metadata into the planner and fail closed when it is missing, invalid, too far in the future, or expired.
- Added a bounded provider-expiry timer plus visible-tab resync so background timer throttling cannot preserve stale action guidance. User activity choices and time filters remain editable while stale recommendations are hidden behind explicit localized copy; no weather value, timestamp, score threshold, health claim, fetch behavior, or official-warning semantics change.
- Updated activity regressions with realistic forecast metadata and added a fake-clock 30-second TTL regression that requires score cards to disappear after expiry. `git diff --check` passes; host-local Node/npm are unavailable, so exact-head hosted lint/type/unit/build/browser/CodeQL gates remain mandatory before merge.

### 2026-09-02 01:27 TRT — expire stale day-plan guidance and centralize forecast freshness
- Continued in an isolated worktree from exact main `4c4d2a82e7200ebd3d7730b23f84ae2092e2f327` while activity-freshness and large-text alert branches validated separately; main later advanced via #597 and this branch is rebased before push.
- Audited the primary Day Plan and found its score, best-window, umbrella decision and share action could remain visible after the hourly forecast TTL elapsed. Added a freshness wrapper that removes the modeled decision surface at provider expiry and on visible-tab resync, leaving explicit localized stale-evidence status instead of extrapolating old guidance.
- Extracted the duplicated commute TTL/skew calculation into `getForecastFreshness`, then reused that contract for Day Plan. The helper uses provider `freshForSeconds` or the API's 30-minute forecast fallback, rejects >60s future skew, and returns a bounded expiry delay with a small timer cushion.
- Added deterministic utility coverage for TTL boundary/fallback/future-skew failure plus a fake-clock Day Plan regression proving the share/action surface disappears after a 30-second TTL. `git diff --check` passes; host-local Node/npm remain unavailable, so exact-head hosted lint/type/unit/build/browser/CodeQL gates are mandatory before merge.

### 2026-09-02 01:36 TRT — expire completed route-weather journey guidance
- Continued independently from exact main `b0bfc582815e15558241e09ee65138f91c1fe552` while Day Plan #600 validates on its own rebased head and the Hero forecast-freshness branch remains isolated/unpushed.
- Audited `RouteWeatherPanel` and found an on-demand corridor score/segment result could remain visible indefinitely after the modeled trip had already ended. The API response currently carries no provider-evidence freshness metadata, so this change does not invent one: it uses only the already validated client departure that produced the request plus the API's returned estimated trip duration.
- Route guidance now remains available through the projected trip, then clears its score/segments/better-departure content and shows explicit localized guidance to re-check the corridor. A bounded timeout handles the normal boundary and visible-tab resync catches background timer throttling. Editing route inputs, changing city/language, or starting a new request resets the expiry notice normally.
- Added a fake-clock regression with a one-hour modeled trip that requires the 78/100 result to disappear after the projected arrival boundary. `git diff --check` passes; host-local Node/npm remain unavailable, so exact-head hosted lint/type/unit/build/browser/CodeQL gates are mandatory before merge.

### 2026-09-02 01:52 TRT — remove freshness clock lint noise without changing recomputation semantics
- Continued independently from exact main `116aed2cbb76d442bc594b7f1b661858132bbc57` in isolated `/tmp/hava81-run11-lint-clean`, branch `automation/hava81-hook-deps-0152`, while PR #601 exact-head CI validates separately.
- The newly introduced freshness/clock revision state intentionally forces a render at expiry/time boundaries, but two `useMemo` dependency arrays encoded those render triggers in ways `react-hooks/exhaustive-deps` correctly flagged as unnecessary/opaque. Activity freshness now keeps the revision state write-only because the recalculated `freshness.fresh` value is the actual memo dependency; commute explicitly reads its clock revision inside the memo callback because that revision itself is the intended recomputation signal for `Date.now()`-sensitive planning.
- Product behavior, forecast freshness thresholds, commute/activity scoring, weather evidence, provider semantics and official-warning behavior are unchanged. Focused Activity + Commute tests pass 20/20; ESLint is now warning-free; TypeScript and `git diff --check` pass.

### 2026-09-02 02:02 TRT — expire stale city-comparison decision evidence
- Continued independently from exact main `116aed2cbb76d442bc594b7f1b661858132bbc57` while route-expiry PR #601 and planner-dependency PR #603 validate on separately owned branches.
- Audited `ComparePanel` and found it retained fetched current weather, forecast and optional AQI metadata but never re-evaluated those freshness contracts. A long-lived comparison could therefore keep Hava81 scores, best-time windows, activity guidance and weather metrics actionable after the evidence that produced them expired.
- Comparison rows now fail closed unless current weather, the decision forecast, and any AQI actually used by that row remain fresh. The next evidence deadline schedules one bounded UI-only wake-up, and visible-tab return rechecks after browser timer throttling. Expired rows disappear rather than extrapolate old values; explicit localized stale/partial-stale copy explains why fewer/no cities remain.
- No weather values, timestamps, score thresholds, provider calls, refresh cadence, MGM semantics or official-warning behavior changed. Focused ComparePanel 14/14, ESLint, TypeScript, full coverage suite 59 files / 574 tests, production build/service-worker stamp/all 81 city pages, and `git diff --check` pass.

### 2026-09-02 03:01 TRT — fail closed on stale first-viewport decision guidance
- Continued independently from exact main `b85fc4e0a5370bfe7e95075bf1e0771ad26b98d7` after #603 merged, while its main deployment validates separately.
- Audited `WeatherDecisionField` and found the visible freshness label could say current evidence was stale/invalid while the adjacent “Plan için öne çıkanlar” list continued rendering actionable modeled guidance from old current/hourly inputs. The field also had no forecast metadata contract, so hourly guidance could survive past the provider forecast TTL.
- The production call now supplies the forecast metadata already owned by `useForecast`; the decision surface fails closed when current evidence is invalid/stale or forecast evidence is missing/expired, while the raw current readings and stale label remain visible. A forecast-expiry timer plus visible-tab resync removes guidance at the provider boundary without altering weather values, provider timestamps, thresholds, fetch cadence, MGM semantics, or official-warning behavior.
- Added fake-clock coverage proving a 38°C modeled heat action disappears after a 30-second forecast TTL and strengthened the future-timestamp regression to require unavailable guidance. `git diff --check` passes; host-local Node/npm are unavailable, so exact-head hosted lint/type/unit/build/browser/CodeQL gates are mandatory before merge.
- Hosted #608 CI exposed two validation-only defects without weakening the production safety change: the shared WeatherDecisionField fixture constructed `fetchedAt` before fake time was installed, making four legacy action-copy tests correctly fail closed as future/invalid evidence, and ESLint flagged the nullable freshness object as an effect dependency. The fixture now uses the suite's explicit 13:00Z evidence time and the effect depends only on stable `forecastExpiresInMs` plus `forecastMeta`; the fail-closed product contract is unchanged.


### 2026-09-02 03:25 TRT — align first-viewport freshness metadata with upgraded hourly evidence
- Started from exact main `8b7576dcf18c8411782934980ddd88146ae2acc9` after #608 merged and main CI/CD #1457 + CodeQL #345 completed successfully; observer production checks remained healthy on root/İstanbul/API/CORS with nginx on 4002.
- Audited `useForecast`/`App` evidence flow and found the decision field consumed `forecast.hourly` (upgraded to the dedicated one-hour source when available) but still received baseline `forecast.meta`. A stale baseline provider snapshot could therefore fail closed fresh first-viewport guidance even while fresh one-hour evidence was actually driving the decisions.
- `WeatherDecisionField` now receives `forecast.displayMeta ?? forecast.meta`, matching the same preferred evidence metadata already used by the other decision surfaces. Added an App integration regression with deliberately stale baseline metadata plus fresh hourly metadata, requiring near-term guidance to recover after the hourly upgrade.
- No weather values, timestamps, thresholds, fetch cadence, MGM semantics, official-warning behavior, API runtime, or production ports changed. `git diff --check` passed; host-local Node/npm are unavailable, so exact-head hosted lint/type/unit/build/browser/CodeQL gates are required before merge.
...
### 2026-09-02 04:01 TRT — fail closed stale first-viewport current observations
- Built independently from exact main `d66ebb30e6a9a4a2c10ed7a9c25062bd35b60d44` while main deployment and other branch gates continued.
- Audited the primary `WeatherDecisionField`: modeled decisions already fail closed at current/forecast expiry, but current temperature, condition, feels-like, humidity and wind remained visible after the current provider TTL. The header did say `Eski veri`, yet the numeric observation block still looked usable.
- Introduced an explicit `currentEvidenceFresh` boundary. Expired/invalid/future-skewed current evidence now hides current observation values behind localized stale-current status; humidity/wind become unavailable. Forecast-derived daily range is independently suppressed when explicit forecast metadata is stale, while independently fresh AQI is not unnecessarily removed.
- Extended the exact-boundary fake-clock regression to require the stale status and absence of old temperature/condition. No provider value, timestamp, fetch cadence, MGM semantics, score threshold, API runtime, or production port changes. `git diff --check` passes; hosted exact-head gates remain mandatory before merge.

### 2026-09-02 03:54 TRT — remove untimestamped saved-city weather from navigation
- Built independently from current main `11930a661aebd9c0fb3997648a79da36ef21ea49` while main deployment and other exact-head gates continued.
- Audited saved-city persistence and confirmed favorites can retain the temperature/icon from a previous visit without any provider `fetchedAt` or TTL. Those values were rendered in the saved-city tab and its accessible name as though they were current weather.
- Kept the backward-compatible persisted shape intact but changed `CityTabs` to present only stable province navigation evidence (plate + city name). Legacy cached icon/temperature are no longer rendered or announced until a freshness-aware saved-city weather contract exists.
- Updated the existing localized-provider-spelling regression to prove legacy temp/icon do not leak into the tab while active-city identity remains correct. An initial broad text patch did not match the evolved component and was abandoned; the accidentally overwritten test file was restored from branch HEAD before targeted safe edits were applied. `git diff --check` passes; hosted exact-head gates remain mandatory before merge.

### 2026-09-02 03:49 TRT — stop stale map temperature markers from looking current
- Continued independently from exact main `f17013109cca009790bf059b9f57a6b1d90e7146` while #610/#611 validate on separately owned branches.
...
### 2026-09-02 provider-evidence refresh alignment
- Audited `useWeather` after the current-weather freshness utility landed and found resume/online refresh still keyed to browser receipt time. A provider-cached response that was already expired could therefore remain unrefetched for up to another five minutes even though freshness-aware UI had hidden its evidence.
...
### 2026-09-02 07:48 TRT — comparison freshness uses one clock snapshot per evaluation
- After #631 merged as exact main `48c2954d76b192abf33ed1970e4c31a89b1b6efc`, continued in a separate branch/worktree rather than mutating the merged PR branch or the dirty primary checkout.
- Audited `ComparePanel` and found each current/forecast/AQ freshness helper could sample a slightly different wall clock while filtering rows and again while calculating the earliest expiry timer. At a shared TTL boundary that could make one evidence source stale while another still used an earlier clock sample, or schedule a timer from a different instant than the row-validity pass.
- Comparison row filtering now captures one `Date.now()` snapshot and passes it to all three shared freshness contracts; the expiry scheduling pass likewise captures one snapshot and uses it across every contributing evidence source. No provider value, TTL, scoring threshold, modeled recommendation, MGM semantics, fetch cadence, or API behavior changes.
- `git diff --check` passes. Host dependency trees remain intentionally absent under disk pressure, so exact-head hosted lint/type/unit/build/browser/Lighthouse/CodeQL gates are mandatory before merge.
### 2026-09-02 07:50 TRT — preserve the current-observation expiry wake-up at the exact TTL boundary
- Continued independently from exact main `48c2954d76b192abf33ed1970e4c31a89b1b6efc` while its production pipeline and comparison-freshness PR validate separately.
- Audited the shared current-weather freshness helper and found an exact-boundary mismatch: an observation at exactly `fetchedAt + TTL` is still classified fresh (`age <= TTL`) but `expiresInMs` was null because the remaining delay check required `> 0`. A render landing exactly on that boundary could therefore expose fresh current evidence without scheduling the 100 ms expiry re-check used by long-lived UI surfaces.
- Aligned the timer contract with the already-inclusive freshness boundary by retaining the 100 ms wake-up when `remainingMs === 0`. Added a deterministic explicit-clock regression for a 30-second TTL boundary. No weather values, TTLs, timestamps, provider semantics, MGM behavior, fetch cadence, scores, or user guidance change.
- `git diff --check` passes; hosted exact-head lint/type/unit/build/browser/Lighthouse/CodeQL remain mandatory before merge because local Node dependencies are intentionally absent under disk pressure.


### 2026-09-02 08:00 TRT — bound transport GET cache by provider evidence expiry
- Continued in an isolated current-main worktree while #633 reruns exact-head gates. Audited the generic browser HTTP GET cache against the provider-freshness contracts already used by weather consumers.
- The transport cache previously retained every successful GET for the browser-wide cache TTL (default 5 minutes) even when a weather payload's `meta.fetchedAt + freshForSeconds` expired earlier. A higher-level refresh could therefore correctly decide provider evidence was stale, call the service, and still receive the old response from the transport cache without reaching the BFF.
- Cache entries now carry an explicit `expiresAt`; when a response exposes valid structural `meta.fetchedAt` plus positive finite `meta.freshForSeconds`, the transport expiry is the earlier of the normal client TTL and the provider evidence expiry. Responses without usable evidence metadata retain the existing client TTL. Already-expired provider evidence is not retained in the transport cache.
- Added a deterministic regression showing a 60-second provider window forces a second network GET after 60.001 seconds even though the generic cache TTL is longer. No provider TTL, weather value, API endpoint, retry behavior, MGM semantics, or user guidance changes.
- `git diff --check` passes. Hosted lint/type/unit/build/browser/Lighthouse/CodeQL gates remain mandatory before merge because host dependency trees remain intentionally absent under disk pressure.


### 2026-09-02 08:16 TRT — fail open to network instead of caching malformed provider freshness metadata
- Continued from exact main `1f1c919e3a9f4e8237a150e1e19e4385cf852040` in an isolated worktree while that revision deploys and #634 remains independently green.
- Audited the new transport-cache provider expiry guard against weather payload validation. `httpClient` stores successful JSON before `weatherService` revives/validates it, so a response with malformed provider freshness metadata could be rejected by the service yet remain replayable from the generic browser cache for the client TTL, delaying recovery after the BFF/provider returns a corrected response.
...
### 2026-09-02 07:56 TRT — reject provider-stale current weather during initial cache restore
- Continued from exact main `48c2954d76b192abf33ed1970e4c31a89b1b6efc` in a third isolated worktree while #632/#633 validate independently.
- Audited `useWeather` after provider-evidence refresh alignment and found the visibility/online refresh path correctly prefers provider `fetchedAt + freshForSeconds`, but the initial localStorage restore still accepted cache solely from browser receipt age. A response already stale at the provider could therefore be written with a new cache timestamp, restored on the next load, and skip the initial network fetch until some later visibility/connectivity event.
- Initial cache acceptance now reuses `isWeatherResultStale(..., now)`, which in turn uses the shared current-weather provider freshness contract when metadata exists and retains client receipt age only as the compatibility fallback. Updated the date-revival fixture to carry fresh provider metadata and added a regression proving a newly persisted but provider-expired observation is rejected and refreshed.
- No weather values, TTLs, timestamps, provider selection, MGM semantics, fetch endpoint, API runtime or user guidance changes. `git diff --check` passes; hosted exact-head lint/type/unit/build/browser/Lighthouse/CodeQL are mandatory before merge under host disk pressure.


### 2026-09-02 09:16 TRT — lock top-level transport freshness selection and malformed-evidence recovery
- Continued independently from exact main `d1e59b0f77185b93845d58bdc4c2968f37150c58` while its post-#640 pipeline validates.
- Added transport-cache regressions for modeled context payloads whose provider freshness lives at the top level while unrelated nested `meta` is also present. The test requires top-level `fetchedAt` / `freshForSeconds` to remain the cache authority instead of being shadowed by non-freshness metadata.
- Added a second recovery regression proving materially future top-level freshness evidence is returned once but not retained, so the immediately following request reaches the network and can accept corrected evidence.
- No weather values, timestamps, TTLs, provider selection, MGM semantics, user guidance, API runtime, or production ports change. `git diff --check` passes; host-local dependency trees remain intentionally absent under disk pressure, so exact-head hosted lint/type/unit/build/browser/Lighthouse/CodeQL gates are mandatory before merge.
- Ran the hardened merged-worktree cleanup in dry-run first, then apply: two clean merged linked worktrees were removed with branch refs preserved; no production/container/runtime resources were touched. Root remains about 90% used / 4.6 GiB free.


### 2026-09-02 09:22 TRT — verify cached Pages shells remain boot-asset coherent during propagation
- After #641 merged as main `78218dfb240be04665d69afc86f38326a488c3d9`, direct production smoke briefly observed a normal root HTML response referencing `SettingsContext-CqlQ39mj.js` while that hashed asset returned 404. A repeat moments later served the current coherent shell (`SettingsContext-tBaG03Ey.js`) and all six root boot assets returned 200, confirming a transient GitHub Pages HTML/asset propagation race rather than a persistent app failure.
...
### 2026-09-02 09:25 TRT — observer now detects HTML-200 / boot-asset-404 production failures
- Followed the transient Pages propagation incident with an observer audit. The five-minute read-only observer previously considered the frontend healthy when `/` and `/istanbul/` returned 200, so an HTML shell referencing a missing hashed JS/CSS asset could remain classified HEALTHY even though a browser might fail to boot.
- Extended the observer's HTML response handling just enough to parse same-origin `/assets/` script/link references from the actually served root shell, then verify each referenced boot asset with bounded read-only GETs. Missing/malformed/no boot evidence now fails the `boot_assets_200` production check; the compact status command reports count and failed paths.
- The observer caps fan-out at 32 asset references and records only failure summaries, not HTML bodies. No repository/production mutation, weather fetch, provider value, API traffic switch, or user-facing runtime behavior is introduced.
- Local gates: observer suite 24/24, Python compilation, and `git diff --check` pass. The live host observer is not replaced until this branch passes exact-head hosted gates and is merged.

### 2026-09-02 10:21 TRT — extend boot-asset observation to the core city shell
- Continued from exact main `1c86758305ddcb8b1fac83be42e6ba1c37f71a97` in a separate worktree while that revision's main pipeline validates; the merged #643 branch was not reused for a second workstream.
- Audited the new production boot-asset observer and found it fetched both `/` and `/istanbul/`, but only extracted/verified hashed assets referenced by the root HTML. A propagation/cache split affecting only the core city shell could therefore still pass `istanbul_200` plus root boot-asset health while delivering a city HTML generation whose referenced chunk was missing.
- `collect_boot_assets` now evaluates the union of root and İstanbul shell assets, requires both shells to expose boot evidence, and reports per-shell counts while retaining the existing bounded 32-asset fan-out. Added a regression proving a city-only missing hashed asset fails closed.
- Dependency-free local gates on exact main: observer suite 25/25 and `git diff --check` pass. No weather/API/provider semantics, production traffic, Pages cache behavior, or API ports change. Hosted exact-head gates remain mandatory before merge.

### 2026-09-02 10:29 TRT — make observer deployment atomic without pausing its timer
- After #643 reached production, the first guarded deployment attempt intentionally failed before copying anything because the oneshot service's `RuntimeDirectory` (and therefore `/run/hava81-worker/observer.lock`) does not persist between collections. This exposed that a deployment script must not assume the runtime lock path exists while the service is idle.
- Production #643 was then installed safely from exact merged main `1c86758305ddcb8b1fac83be42e6ba1c37f71a97` using same-filesystem temp files plus atomic `mv`; the five-minute timer remained enabled/active throughout. Installed collector/status hashes match the merged revision and a forced collection reported healthy root/API/CORS/nginx plus boot assets 6/6.
- Prepared a versioned `deploy/oracle/install-observer.sh` on current main `884c347631466ffdc705150f84ddcf8c90afe314`. It runs the observer suite and in-memory Python syntax compilation before writes, stages collector/status/systemd units beside their targets, atomically replaces each file without stopping/disabling the timer, reloads systemd, runs one collection, verifies timer enabled+active, and prints status.
- Added hosted `bash -n` validation for the installer and updated observer documentation. Local dependency-free gates: observer suite 25/25, installer `bash -n`, no generated `__pycache__`, and `git diff --check` pass. No weather/API/provider semantics, traffic routing, or API ports change.

### 2026-09-02 10:37 TRT — bound GitHub compare payloads so API drift does not become falsely unknown
- Fresh post-#652 observer state exposed `api_deploy_unknown=true` even though the compare lookup returned HTTP 200. Directly reproduced the cause: comparing deployed API revision `7368bef...` to current main produced a 1,773,446-byte default GitHub compare response (250 commit objects + 136 changed files), exceeding the observer's intentional 1.5 MB HTTP read bound; JSON parsing therefore could not supply `files`.
...
### 2026-09-02 11:51 TRT — flatten Forecast Atlas into an editorial evidence surface
- Continued from exact main `541abea2296cc60744631494ac71edc0b04ac6be` in isolated branch `automation/hava81-forecast-editorial-1149` while the post-#660 main pipeline and observer PR reruns proceed independently.
...
### 2026-09-02 13:58 TRT — do not carry stale daily rows through partial same-city recovery
- While the post-#667 main pipeline validated, audited `useForecast` partial-recovery behavior on current main `036fe0b80c6c662fc51dba596241e0472a0ba6a3`.
- Found a provenance mismatch on same-city refresh: if the resilient baseline request failed, the dedicated hourly request succeeded, and that hourly response did not include its optional daily series, the hook retained the previous daily rows while replacing the visible hourly metadata with the new source. Old daily evidence could therefore appear alongside fresh hourly provenance.
...
### 2026-09-02 — clarify the root location-choice hierarchy
- Continued from exact main `22880f680326f74bde99488d33d354ea9d15e802` after dark-action contrast reached production. The root location gate still rendered three actions with comparable visual weight on wider screens, weakening the intended permission-first decision flow.
- Converted only the location-gate action area into an editorial decision grid: location is the full-width primary row, İstanbul fallback and “Başka şehir ara” share the secondary row on wider screens, and all three stack below 30rem. Added block-axis rules around the onboarding surface without changing copy or behavior.
- Bound the primary location action to the existing privacy note through `aria-describedby` and added a desktop browser regression for primary/secondary geometry, privacy association, zero eager geolocation calls, and page containment. The existing 320px/200%-text mobile regression remains authoritative for narrow layouts.
- No provider, weather, location precision, fallback semantics, routing, or API topology changed. Host disk remains under the preferred free-space threshold, so exact-head hosted frontend/API/build/browser/Lighthouse/CodeQL are mandatory before merge rather than installing another local dependency tree.

### 2026-09-02 17:45 TRT — add fail-closed stale clean worktree hygiene

- Fresh observer state reported production healthy on root/İstanbul/readiness/boot-assets/CORS with OpenWeather circuit closed and nginx still on 4002; host root disk was critical at 97.6% used with ~1.08 GiB free.
- Ran the existing hardened cleanup in default mode after fetching current main; it removed only two clean merge-equivalent linked Hava81 worktrees, preserving their branch refs and all runtime/Docker state. Disk remained ~98%, confirming the remaining pressure is not safely recoverable through merge-only Hava81 cleanup alone.
...
### 2026-09-02 18:50 TRT — expire AQI evidence independently in the decision hero

- Fresh observer state at 18:41 TRT showed production healthy (root/İstanbul/readiness/boot assets/CORS, OpenWeather circuit closed, nginx 4002) while root disk remained critical at ~97% used / ~1.4 GiB free. Safe Hava81-only storage cleanup found no eligible standalone clones and only one permission-blocked ~1.5 MB stale linked checkout, so no unrelated runtime/browser/user data was touched.
- Audited the first decision surface and found that `AirQuality.meta` carries its own provider timestamp/TTL but the AQI metric and AQI-derived guidance ignored that freshness boundary.
- Generalized the existing current-evidence freshness helper to accept the shared `WeatherDataMeta` shape, freshness-gated AQI independently, and scheduled the hero to rerender at the AQ observation's exact TTL boundary as well as the current-weather boundary/minute tick.
...
### 2026-09-02 20:49 TRT — merge AQ handoff guard and make zero-candidate disk cleanup diagnosable

- Fresh observer state showed production healthy (root/İstanbul/readiness/boot assets/CORS, OpenWeather circuit closed, nginx 4002) and PR #692 exact head `bf190a9798470e7d5b7610c1aaf9d6bbbdecb0ea` green in CI/CD #1679 and CodeQL #567. Re-verified mergeability and exact head, then merged #692 to main as `ae93db6518ad7cffed4388314540dab183606d17`.
- Post-merge main CodeQL #568 is green; CI/CD #1680 is still in progress and remains a required production gate. Continue independent work while it runs, then smoke-test production once green.
- Root disk remains critical at ~97.2% used / ~1.27 GiB free. Hava81's hardened linked-worktree/artifact cleaner and archive-first standalone-clone cleaner both found zero eligible removals. Direct sizing shows all `/home/ubuntu/hava81-*` checkouts together are ~929 MiB, while the dominant host consumers are outside the Hava81 cleanup boundary (`/var/lib/containerd` ~11 GiB, `.hermes` ~7.6 GiB, `.hermes_fresh_backup` ~2.1 GiB, plus unrelated Postify/Nexus/user data). No unrelated runtime, browser, user, or active container data was removed.
- Added a read-only `--audit` mode to `cleanup-stale-standalone-checkouts.sh`. It does not change eligibility or delete anything; it reports aggregate candidate classes so a zero eligible result is no longer opaque. Real-host audit currently sees 203 matching Hava81 directories: 42 standalone clones, 154 linked worktrees, and 7 directories without Git metadata; still zero standalone clones qualify for safe archive/remove.
- Validation passes: both cleanup scripts `bash -n`, standalone cleanup safety contract PASS, and `git diff --check`. Next: commit/push/open the audit PR from `automation/hava81-disk-candidate-audit-2048`, require exact-head hosted gates, re-check main #1680, and smoke production when green.


### 2026-09-02 20:56 TRT — reclaim across Hava81 worktree registries and close owner-mismatch blind spot

- PR #693 exact head `8c6963ce47b81ca8b5882e6699542cf205cf26d4` passed CI/CD #1681 and CodeQL #569 and was merged as main `5ecc721a9c772ef8b160a5a918c65eab072c985c` after a fresh healthy production observer snapshot.
- The read-only audit exposed that Hava81 checkouts are spread across ten Git worktree registries, not one. Running the already-merged hardened cleaner from the nine secondary registries with fresh `origin/main` refs found and removed 31 positively eligible clean/stale-clean linked checkouts plus two rebuildable artifact directories; branch refs were preserved and every apply path rechecked eligibility. A second pass from the primary registry as the checkout owner removed three more eligible worktrees and two rebuildable artifacts. Root free space rose from ~1.27 GiB to ~1.4 GiB; disk pressure remains active.
- Root-run inspection still skipped many Ubuntu-owned worktrees because Git rejected `git -C <worktree>` as dubious ownership. Added exact-path, per-command `safe.directory` overrides only to the cleaner's read-only status/HEAD/symbolic-ref inspections. This does not modify global Git trust and retains fail-closed behavior on any inspection error.
- Existing cleanup safety contract and shell syntax validation pass, and `git diff --check` passes. Branch was rebased onto post-#693 main before documenting. Next: push/open PR for the owner-mismatch fix, require exact-head CI/CodeQL, then use the merged version to re-audit remaining primary-registry worktrees under root if green.

### 2026-09-02 20:59 TRT — expose the observer's already-computed absolute disk usage

- While PR #694 validates independently, re-audited current-main observer output against its implementation. `collect_host()` already computed `used_bytes = total_bytes - free_bytes`, but the value was accidentally omitted from the returned `host.disk` object, so the live state could not expose the absolute consumption promised by the earlier observability work.
- Added `host.disk.used_bytes` without changing thresholds, health semantics, production checks, or any runtime mutation. Extended the disk regression to assert `used_bytes + free_bytes == total_bytes`.
- Full dependency-free observer suite passes 26/26 and `git diff --check` passes. Next: push/open this isolated branch, require exact-head hosted gates, and rebase after any earlier operations PR that reaches main first.

## 2026-09-02 21:13 TRT — remove inherited vertical flex basis from mobile Activity Planner
- Started from exact clean `origin/main` `1ea10d78e499d7e7f49a730e92dc9686affbf1d4` in isolated `/home/chatgpt/hava81-auto-run11-2110`, branch `automation/hava81-run11-2110`; preserved the unrelated dirty/stale `/home/chatgpt/hava81-rain-detail-2135` worktree unchanged.
- Mobile audit found `.activity-planner__header` switches to `flex-direction: column`, while `.activity-planner__sensitivity` still inherited desktop `flex: 0 1 12rem`. In column layout that 12rem basis becomes vertical, creating avoidable empty/tall header space. The mobile breakpoint now overrides only `flex-basis: auto`; desktop behavior is unchanged.
- Extended the existing 320px + 200% text Playwright regression to assert the mobile sensitivity basis resolves to `auto` and to bound sensitivity/header heights in addition to existing no-overflow checks.
...
## 2026-09-02 21:28 TRT — patch applicable Fastify trustProxy advisory
- Security audit of the repository's single open Dependabot alert found runtime `fastify` 5.10.0 affected by `GHSA-3m5p-2c4r-xxw2` / `CVE-2026-16732` (`>=5.8.3 <5.12.1`), X-Forwarded-* spoofing under hop-count trustProxy.
- Applicability is confirmed: `apps/api/src/app.ts` uses `trustProxy: 1` in production behind the single Nginx reverse proxy hop. This is not being dismissed as unreachable code.
- Prepared isolated branch `automation/hava81-run11-2127-next` from exact main `605bea4409f51579f1568a7532c478839bcb361a`: raise Fastify to `^5.12.1`, refresh its lockfile record/integrity, resolve `process-warning` to 5.1.0, and replace the now-disabled numeric `trustProxy: 1` with explicit `127.0.0.0/8`, `::1/128`, and Docker bridge `172.16.0.0/12` immediate-peer trust. The API host port remains loopback-only and the Nginx topology is unchanged.
- Registry metadata was independently checked for Fastify 5.12.1 and process-warning 5.1.0. Added a regression that rejects X-Forwarded-* spoofing from an untrusted direct `203.0.113.10` peer while accepting forwarded metadata from a representative trusted Docker bridge peer (`172.20.0.1`). Local JSON/lock consistency assertions and `git diff --check` pass. Node/npm remain unavailable locally, so hosted `npm ci`, API type/test/build and audit gates are mandatory.
- This is an API runtime dependency change. If merged, deploy only through the existing blue/green path: keep production on 4002 until the new canary passes readiness/CORS/core-city checks, retain 4001 as rollback/canary as appropriate, and switch traffic only after validation.
- Next action: commit/push/open the security PR, wait only in the sense of not merging it before exact-head gates; continue independent work and re-check PR #697/#698 hosted runs meanwhile.
## 2026-09-02 21:25 TRT — keep forecast/activity rail focus indicators visible
- Started from exact post-#696 main `605bea4409f51579f1568a7532c478839bcb361a` in isolated `/home/chatgpt/hava81-auto-run11-2124-focus-rails`, branch `automation/hava81-run11-2124-focus-rails`, while PR #697 runs separately.
- Audit found two horizontally scrollable choice rails still used positive/outside focus outlines: Forecast Atlas interval buttons explicitly used `outline-offset: 2px`, and mobile Activity Planner chips inherited the global +2px offset. At the first/last rail edge those indicators can be clipped by the scroll viewport.
- Forecast interval focus now renders at `-2px`; mobile Activity Planner chip focus gets the same inward treatment. Added a mobile Playwright regression that exercises actual keyboard modality (Tab then Shift+Tab), confirms focus returns to each control, and verifies each `overflow-x: auto` rail presents a visible >=2px non-positive-offset outline.
- `git diff --check` passes; local Node/npm remain unavailable on this disk-pressured host, so hosted exact-head frontend/browser/Lighthouse/CodeQL gates remain mandatory.
- Production after #696 was directly smoke-tested: `/` and `/istanbul/` returned 200 and all six boot assets referenced by each shell returned 200; API readiness returned 200, fresh timestamp, `Cache-Control: no-store`, correct Hava81 CORS origin, OpenWeather circuit `closed`.
- Next action: commit/push/open this independent focus-rail PR; do not merge before exact-head gates. Continue #697 failure correction in its own worktree and require a fresh observer check immediately before either merge.

## 2026-09-02 22:00 TRT — close the remaining Forecast Atlas scroll-region focus edge
- After #699 was merged, exact-main CI/CD #1697 and CodeQL #585 passed; the patched Fastify API was deployed through the blue/green path to validated slot 4001, with 4002 retained healthy as rollback. Public readiness/CORS/core-city and root/İstanbul smokes passed.
- #698 exact head `ee20d57b6244834d558662992e9720a7db7670ed` passed CI/CD #1698 and CodeQL #586 and was merged as main `ba695a39f840419939258891c6ddc9851d27e07a` after fresh production verification.
- While post-merge main CI runs, a follow-up keyboard audit found the Forecast Atlas hourly viewport itself remains focusable and horizontally scrollable but still renders its 2px focus indicator outside the rail boundary. Prepared an isolated `outline-offset: -2px` correction and extended the existing narrow browser regression to require a visible inward focus indicator on that real region.
- Host disk remains pressured but improved to about 95% used / 2.4 GiB free after removing only the automation user's rebuildable npm/node compile caches and 95 MiB of unused Docker build cache; active containers, rollback image, other projects, `.hermes`, and user/browser data remain untouched.
- Next: `git diff --check`, commit/push/open the isolated hourly-focus PR, require exact-head hosted frontend/browser/Lighthouse/CodeQL; meanwhile verify main #1699 and post-merge public boot assets before any further merge.

## 2026-09-02 22:24 TRT — carry modeled-warning provenance into browser notifications
- Fresh observer state at the start of the run showed production healthy (root/İstanbul/readiness/boot assets/CORS, OpenWeather circuit closed) with no open automation PRs; host disk pressure remained active at ~95% used / ~2.5 GiB free.
- Ran the hardened Hava81 worktree cleanup from current main. The normal merge-equivalent set was exhausted, then the previously approved opt-in stale-clean path at a positive 6-hour threshold revalidated and removed 22 clean attached linked worktrees in total across two passes while preserving every branch ref. No Docker runtime, browser/user data, `.hermes`, other projects, or dirty/detached/current worktree was touched.
- Trust/safety audit found that the alerts panel visibly says its alerts are Hava81 modeled guidance rather than official MGM MeteoUyarı, but the browser notification payload itself did not carry that provenance. A background notification such as “Kuvvetli rüzgâr” could therefore lose the distinction once separated from the panel UI.
- Added localized notification-only disclosure copy in Turkish and English and append it to every delivered browser notification body. Added a regression that exercises service-worker delivery and requires the payload itself to contain both “Hava81 model rehberi” and “resmî MGM MeteoUyarı değildir”. No weather value, threshold, provider interpretation, or official-warning claim is changed.
- `git diff --check` passes. Node/npm are intentionally unavailable on this disk-pressured host, so exact-head hosted frontend/unit/browser/Lighthouse/CodeQL gates are mandatory before merge.
- Next: commit/push/open this bounded trust-clarity PR, continue independent audit while hosted gates run, then fresh-observer + exact head/mergeability verification before merge and public production smoke afterward.

## 2026-09-02 22:38 TRT — make standalone disk-cleanup blockers observable without widening deletion
- Root disk remains the active host incident (~94.5% used, ~2.5 GiB free) after safe linked-worktree cleanup. The standalone cleaner correctly returned zero deletion candidates but its prior audit only counted checkout classes, leaving the reason for zero eligibility opaque.
- Added read-only aggregate exclusion reasons to `cleanup-stale-standalone-checkouts.sh --audit` without changing `candidate_is_eligible`, archive-ref creation, removal behavior, or any apply gate. The audit reports eligible/dirty/origin-mismatch/detached/unrepresented/recent/in-use/unreadable/archive-conflict counts and never prints file contents.
...
## 2026-09-02 22:53 TRT — enforce the standalone cleanup audit read-only contract
- While post-#702 main CI runs, reviewed the real-host standalone cleanup output after using `--apply --audit` and found a safety-contract mismatch: help text promises `--audit` is read-only, but combining it with `--apply` still performed archive-first removals and then printed the pre-action audit snapshot.
- Changed argument validation to reject `--apply --audit` before scanning or mutation. Operators can still run a read-only audit first and a separate explicit `--apply` after reviewing candidates; deletion eligibility, exact-HEAD archive refs, rechecks, and fail-closed exclusions are unchanged.
- Extended the dependency-free safety fixture to prove the rejected combination neither removes the eligible checkout nor creates its archive ref. `bash -n` for both scripts, the full standalone cleanup safety contract, and `git diff --check` all pass locally.
- Branch/worktree: `automation/hava81-run11-cleanup-postapply-audit` / `/home/chatgpt/hava81-cleanup-postapply-audit`, based on post-#702 main `33e7e2ba95d69c9d5b4048f6ccf53da8f94ab01b`. Next: exact-main/lease recheck, commit/push/open bounded ops-safety PR, require exact-head hosted gates, and keep post-#702 production validation independent.

## 2026-09-02 23:05 TRT — align standalone cleanup audit with registered-checkout safety
- Post-#704 audit found a second diagnostics-only mismatch: `candidate_is_eligible` already excludes the repository itself and registered checkouts, but `audit_reason_for_standalone` did not. A clean primary clone under the scanned `hava81-*` parent could therefore inflate `eligible` even though mutation would refuse it.
- Added an explicit `registered` audit reason before content/age checks and surfaced its aggregate count. Mutation eligibility, archive creation, and removal logic are untouched.
- Extended the dependency-free fixture so the primary repository itself is scanned under a matching `hava81-*` name and must report `eligible=0 registered=1`, while the existing candidate matrix still reports one true eligible standalone clone. `bash -n`, the full standalone cleanup safety contract, and `git diff --check` pass.
- Isolated branch/worktree: `automation/hava81-run11-cleanup-audit-registered` / `/home/chatgpt/hava81-cleanup-audit-registered`, based on production-green main `eb732a9b0413b4e07b70e2e0e0cd1c4ab06d33f0`. Next: commit/push/open bounded diagnostics PR, require exact-head hosted gates, then merge only after fresh main/production verification.

## 2026-09-02 23:16 TRT — quantify standalone cleanup blockers without widening deletion
- Merged PR #705 as main `049f1377626bf3c466b2939478a863af741453ee` only after exact-head CI/CD #1712 and CodeQL #600 were green; post-merge main CI/CD #1713 and CodeQL #601 also passed. Public root/İstanbul and all six boot assets returned 200; API readiness was fresh/no-store with OpenWeather circuit closed and exact production CORS. Nginx remains on the previously validated 4001 slot with 4002 retained as rollback from the Fastify deployment.
- Reinstalled the already-merged read-only observer from green main because the host copy lagged the repository: the installer ran the full dependency-free observer suite 26/26, atomically replaced files without disabling the timer, and the fresh state now exposes `host.disk.used_bytes` as intended. Production remained healthy after install.
...
## 2026-09-02 23:35 TRT — scope outdoor-window guidance to weather suitability
- A read-only 390×844 production inspection at 23:14 TRT showed the first decision surface saying roughly “03:00 looks like a calmer window for being outdoors.” The underlying `outdoor-window` calculation scores meteorological conditions only; it does not know the user's sleep schedule, personal safety context, transport, or other non-weather constraints.
...
## 2026-09-03 00:12 TRT — scope city SEO outing guidance to weather evidence
- Fresh observer state showed production healthy while host disk pressure remained active at ~94.6% used / ~2.4 GiB free. PR #712 was green on its previous head but stale against current main; it was rebased in an isolated worktree onto exact main `bf250a98f9bf9f19efefe569ede8b580af471903`, preserving append-only docs, and repushed with an exact lease as `97b4a66247b3f5110ef3094de1638a3687186d3c`.
- While #712 reruns hosted gates, a separate current-main trust-copy audit found city document metadata still advertised the `best time to go out` / `en iyi dışarı çıkma saati` without naming the evidence domain. Tightened only the SEO/document-description copy to `best weather window` / `hava açısından en iyi ... penceresi`; no weather value, score, ranking, threshold, provider interpretation, or runtime decision logic changed.
- Added a localization regression requiring the city metadata recommendation to remain weather-scoped. `git diff --check` is required locally; hosted exact-head frontend/browser/Lighthouse/CodeQL remain mandatory before merge.
- Hardened merged-worktree cleanup revalidated one clean merged linked checkout. Ubuntu ownership could not remove it, so the same already-merged safety script was rerun as root with an exact per-process `safe.directory` override for the current worktree only; it removed that one eligible checkout and preserved its branch ref.

## 2026-09-03 00:22 TRT — align generated city HTML metadata with weather-scoped guidance
...
## 2026-09-03 02:38 TRT — harden cleanup for accumulated Hava81 Chromium audit profiles
- Root disk pressure remained active after worktree/artifact cleanup. Read-only size inspection isolated ~1.0 GB under Snap Chromium's private temp tree in directories named only for Hava81 audit profiles; every candidate was checked against running process command lines before removal. Removed 31 inactive Hava81-owned temp directories / 1,017,454,819 bytes, improving root free space from ~2.2 GiB / 96% used to ~2.9 GiB / 94% used. Generic Lighthouse, Postify, browser-user, container and unrelated temp data were untouched.
- Added `scripts/cleanup-hava81-browser-temp.sh`: explicit root and age are mandatory; dry-run is default; only `hava81-*`, `h81deploycheck`, `h81meta`, and `h81text` direct-child directories can qualify; symlinks, recent trees and process-referenced paths fail closed; eligibility is rechecked immediately before `rm`.
- Added a self-contained shell safety contract proving old allowed profiles are removable while recent, unrelated, symlinked and in-use paths are retained. Wired that test into Frontend quality CI beside the existing worktree cleanup contract.
- Local gates: both shell files `bash -n` PASS, browser-temp safety contract PASS, `git diff --check` PASS. No production/runtime browser profiles, Docker data, weather/API behavior, deployment topology or user data are touched by this repo change.

## 2026-09-03 02:48 TRT — attribute UV protection threshold to WHO
- Fresh observer state remained production-healthy while root disk pressure persisted at ~93.6% used / ~2.9 GiB free. Exact-head CI/CD and CodeQL for PRs #722 and #723 were green and both PRs were mergeable, but the direct GitHub merge mutation was unavailable in this run, so work continued independently rather than idling.
...
## 2026-09-03 05:12 TRT — replace unconfigured Codecov upload with first-party coverage artifact
- Investigated failed PR #733 at the hosted job-log level. GitHub OIDC itself succeeded and Codecov received a 1,926-character token; the upload then failed with `Repository not found`, proving the remaining blocker is external Codecov repository onboarding rather than CI authentication.
- Removed the already-broken non-blocking Codecov upload from the proposed fix path instead of masking or repeatedly retrying an unavailable external repository. Frontend coverage remains generated by `npm run test:ci` and is now uploaded as a required GitHub Actions artifact (`frontend-coverage`, 7-day retention, missing-file failure).
- This keeps coverage evidence attached to every quality run using the repository's existing GitHub trust boundary, without adding secrets, OIDC write scope, application/runtime changes, or a dependency on external Codecov configuration.
- Local diff/YAML checks and exact-head hosted CI remain required before merge. PR #733 should be superseded rather than merged because its fail-closed Codecov experiment correctly exposed the external onboarding gap.

## 2026-09-03 05:24 TRT — lift first-forecast temporal markers out of 10px microtype
- Continued from exact main `942b15e40b94b98cc201088b2c1f06cfbd2190d8` after the first-party coverage-artifact change merged.
...
## 2026-09-03 07:18 TRT — keep forecast evidence attribution readable
...
## 2026-09-03 08:04 TRT — make Activity Planner score context legible beside the number
- Continued independently from exact main `f600baae1be4d23246c1a2ffacaa8bb658aeef73` while #758 exact-head gates ran.
...
## 2026-09-03 21:02 TRT — cover Snap Chromium Hava81 profiles in stale-process observation
- During safe disk hygiene, eight inactive Hava81 audit profiles were positively identified under `/home/ubuntu/snap/chromium/common`, not `/tmp`; the existing observer process filter only recognized `/tmp/hava81-*`.
- Generalized only the read-only process identity check to parse Chromium `--user-data-dir` and accept the same Hava81 profile basenames used by the hardened browser-temp cleanup tool, independent of parent path. Postify/unrelated profiles and non-browser commands remain excluded.
- Both the normal `ps` path and hardened `/proc` fallback now cover Snap profile paths. Added regressions for `--user-data-dir=` and separated-argument forms while preserving age threshold, bounded reporting, and advisory-only warning semantics.
- Local observer suite passes 38/38, worker-status suite 2/2, Python compile and `git diff --check` pass. No process is terminated and no browser/temp data is deleted by the observer. Exact-head hosted CI/CodeQL remain required before merge/install.

## 2026-09-03 21:16 TRT — make the functional microtype regression enforce the floor already shipped
- Continued independently from exact main 5ab80b2e3c80abddead051d8343e17e5cea6ff65 while its post-#841 production pipeline runs.
- Audited the central functional-microtype regression and found its shared helper still allowed an 11px-equivalent floor even though every remaining call site — saved-city province plates, Daily Plan local-day boundaries, and modeled Context Signal units — now ships at the shared 13px-equivalent functional-copy floor.
...
## 2026-09-03 21:34 TRT — encode stable 4002 production / 4001 rollback promotion in the deploy script
- Audited the production blue-green script after restoring the live API to the documented topology and found a structural drift source: when production was on 4002, the script validated 4001 and then ended with public traffic on 4001. That contradicted the existing Hava81 deployment decision to use 4001 as canary/rollback and return validated production to 4002.
- Reworked the deploy flow so a normal 4002 deployment validates the candidate on 4001, captures the previous 4002 image, temporarily switches to the validated canary, promotes that exact candidate image to 4002 without rebuilding, validates 4002, returns public traffic to 4002, then restores the previous production image to 4001 as immediate rollback. If production is already on 4001, the script validates a 4002 candidate and returns directly to the preferred slot.
- Added a hermetic PLAN_ONLY regression for both starting topologies and wired it into the API CI job. Existing API operation-lock, rollback-target and readiness-contract tests also pass; shell syntax and git diff checks pass. This branch has not been executed against production and requires exact-head hosted CI/CodeQL before merge.
- Live production remains healthy on 4002 with 4001 healthy as rollback, exact CORS, fresh no-store readiness, OpenWeather circuit closed, and root disk inside the hard gate at about 91.7% used.

## 2026-09-03 21:48 TRT — raise interactive map province-code readability
- Fresh audit started from exact production/main `f8037229bd70a94b106a21d446b2a0e2ccd84718`; no product PR was open when the UI branch was created. Production root and `/istanbul/` matched `gh-pages` byte-for-byte and observer reported `production.healthy=true`.
...
## 2026-09-03 22:01 TRT — Daily Plan slot-time cascade readability fix
...
## 2026-09-03 22:54 TRT — expose linked-worktree cleanup exclusions during disk incidents
- Re-verified fresh observer state on exact main `e8892520f0ed1347f1a9787431e792976108f9e7`: production remains healthy on preferred API port 4002, frontend revision matches main, OpenWeather circuit is closed, and all six open automation PR exact heads remain observer-green; the only hard gate is root disk pressure.
- Safely reclaimed Hava81-owned storage without deleting source/history: pruned about 95 MB of rebuildable Docker builder cache, normalized mixed root/ubuntu ownership only inside the shared Hava81 `.git`, and reduced about 150 MB of loose Git objects to one about 15.7 MiB pack. A separate stale standalone clone was compacted without deleting its files or refs.
- Fresh observer disk state improved from 93.9% used / 2,955,907,072 bytes free to 93.3% used / 3,226,640,384 bytes free. The configured 92% gate still requires exactly 635,559,281 additional free bytes, so merge/deploy remains intentionally blocked.
- Auditing the linked-worktree cleanup path exposed a reliability gap: status/ownership failures were silently skipped, making a zero-candidate report ambiguous. Added read-only `--audit` output with scanned/dirty/status-unreadable/missing counts and made `--audit --apply` fail closed. The existing cleanup safety contract plus the new audit/mutation guard pass locally; shell syntax and `git diff --check` pass.
...
## 2026-09-04 13:25 TRT — make source-preserving Git compaction repeatable
- During the active root-disk incident, plain Git GC reclaimed measurable Hava81-owned storage from the canonical repository and several preserved standalone clones without deleting dirty source, branches or tags. The operation was useful but previously manual and therefore hard to audit/repeat safely.
- Added `scripts/compact-hava81-git-objects.sh`: dry-run by default, requires an explicit non-root scan directory, considers only direct `hava81-*` standalone repositories whose normalized origin matches the canonical Hava81 origin, and applies plain `git gc` only when explicitly requested. It intentionally does not use `--prune=now`, does not touch linked worktrees or foreign origins, and preserves dirty working trees.
- Added a hermetic regression that proves dry-run is non-mutating, foreign origins are excluded, and apply mode preserves a dirty working file plus exact HEAD. Wired the contract into frontend CI.
- Local validation: both shell files pass `bash -n`, the hermetic compaction regression passes, and `git diff --check` passes. No product/weather/API behavior or production traffic changed.
## 2026-09-04 13:33 TRT — prevent root-owned Git metadata during worktree cleanup
...
## 2026-09-05 00:50 TRT — use date-only semantics for daily forecast labels
- Continued independently from exact main `99f2235be9693cfeabc9c72d9f7cd257ffe20144` in isolated `/home/chatgpt/hava81-auto-run11-0050-next` while the #923 main pipeline validates separately.
...
## 2026-09-05 09:49 TRT — detect Git metadata ownership drift before repository mutation
- During the exact-head #965 merge, the GitHub squash succeeded but the immediately following normal `git fetch` failed because 119 entries beneath the canonical shared `.git` metadata had been left root-owned by an earlier privileged operation. The working tree/source was untouched; ownership was restored only inside `.git`, after which normal fetch succeeded and exact `origin/main` matched GitHub again.
- Added a read-only `audit-git-metadata-writability.sh` guard that checks the shared object fan-out plus refs/logs/worktree metadata as the current uid and fails before a fetch/rebase/cleanup can rely on a repository that is already unwritable. The helper never chowns, deletes, fetches, or changes refs.
- Added a hermetic regression that makes one loose-object fan-out directory non-writable and proves the audit fails closed, then passes again after permissions are restored. CI now exercises this lightweight operational contract alongside the existing source-preserving worktree cleanup guard.

## 2026-09-05 09:58 TRT — cleanup apply inherits the Git metadata fail-closed gate
...
## 2026-09-05 12:42 TRT — run 11 continuity checkpoint after safety-copy rollouts
- Merged #979 from exact head `aec52d14f0b9ba7d354b9c42a22afc8d6a8c4dd2`; resulting main `b9c62aa0f7663d3d6bd69d9a6aad23245f4fe8f0` completed CI/CD run #2398 (`33957769268`) including browser, Lighthouse, Docker and Pages, and public root/İstanbul served the exact revision marker.
- Rebuilt and merged #980 from exact head `86c7c0e7f1718e8f1c159d9911f2e5725567e8c2`; current main is `487b974214bc3718c0feecafbdbac60def5ce4f3`. CI/CD run #2400 (`33958144041`) completed successfully across frontend/API/build/browser/Lighthouse/Docker/Pages and public root + `/istanbul/` serve exact `487b9742…`.
- Repaired only 226 mis-owned entries beneath canonical `/home/ubuntu/Hava81/.git/objects` from `sentinelx` to the repository owner `ubuntu`; the read-only metadata writability audit now passes and owner-level `git fetch origin --prune` works again. No working-tree source or branch content was rewritten. Configured the already-authorized `gh` credential helper for the `ubuntu` Git owner so isolated worktrees can push without privileged Git metadata writes.
- Source-preserving disk audits found no safe standalone clone compaction target, no stale Hava81 browser temp, no meaningful build cache, and only tiny rebuildable node_modules/artifacts. Removed only two disposable `/tmp` clones after their work was safely remote/merged. Normal merge/deploy disk gate reopened; the separate API-build reserve remains about 0.5 GiB short, so API runtime changes stay held.
- Rebuilt stale API PRs onto exact current main: #981 head `cc95cfce087f982e6cbfc57f35dc05c150ba5561` (OpenWeather blank-text trust boundary) and #982 head `f52d654517b0723d99d68e0ae43cd9755e833cdb` (API error `no-store`). Both replacements have exact-head CI/CD + CodeQL + browser + Lighthouse green. Closed superseded stale #962/#963. **Do not merge #981/#982 until fresh Oracle state reports `api_build_headroom_ok=true`; then use the 4001 canary/rollback → validated 4002 stable blue-green path.**
- Prepared #983 from exact main: branch `automation/hava81-score-stable-copy-run11-1242`, head `3929eb1be1fd83febd4796151f89c96ff18224ca`, softening the remaining score-explanation favorable-risk wording. API/CodeQL/build are already green; browser/Lighthouse were still pending at this checkpoint.
- Prepared #984 from exact main: branch `automation/hava81-observer-rate-fallback-run11-1244`, head `2320f7473b9553a317cde6a5085da44e4a1197be`. It prevents the observer's smaller Actions-runs fallback from retrying deterministic 4xx responses such as anonymous GitHub rate-limit 403; local observer suite 47/47 + diff-check pass. Hosted CI is pending. The current anonymous observer quota was exhausted during this tool-heavy run; authenticated GitHub reads still have ample quota and production itself is healthy. Keep fail-closed behavior until the anonymous window resets.
- MGM research found the first-party MeteoUyarı page calls `https://servis.mgm.gov.tr/web/meteoalarm/today` and `/tomorrow`, but direct server access returns `Not allowed by MGM` and browser access is CORS-scoped to `https://www.mgm.gov.tr`. Do not spoof MGM Origin/Referer or ship this restricted endpoint; retain modeled Hava81 guidance separately from official MGM warnings.
- Current checkpoint worktree: `/home/ubuntu/hava81-run11-checkpoint-1236`, branch `automation/hava81-run11-checkpoint-1236`, base `487b974214bc3718c0feecafbdbac60def5ce4f3`.
- Next queue: (1) exact-head/fresh-gate merge #983 when fully green, smoke new main; (2) exact-head/fresh-gate merge #984 when fully green, install the observer only from the resulting exact main and verify the next observer cycle; (3) keep #981/#982 green/rebased but blocked until API-build disk reserve is truly green; (4) continue bounded UX/accessibility/performance/trust-boundary work while external gates run; (5) merge this append-only checkpoint after rebasing onto the newest stable main and preserving all intervening checkpoints.

### 2026-09-05 12:52 TRT — concurrency + disk follow-up
- #983 passed exact-head CI/CD + CodeQL/browser/Lighthouse and merged from `3929eb1be1fd83febd4796151f89c96ff18224ca`; main became `bd1903afec5b29fb1a84bd4433fc052c11da6930`, CI/CD #2405 (`33958694597`) completed fully green, and public root/İstanbul served exact `bd1903af…`.
- Another autonomous workstream prepared #986 (`baec6906b530b9958dc8d3852e96d315455b4baf`) fixing `useLocalStorage` runtime key rehydration. Reviewed its two-file + test diff, confirmed all hosted gates green, fresh normal disk gate and production 4002/OpenWeather readiness, then merged with expected head. Current rollout target after this checkpoint is main `e8fd19428f61b35b74215f7bdd4324fa63c293a0`, CI/CD #2409 (`33959097636`).
- The parallel observer PR #985 contained the same observer logic but accidentally changed `observer.py` mode 100755→100644. Closed #985 without deleting its branch/ref. Reopened the mode-preserving current-main rebuild as #987; after lease-safe rebase its exact head is `78a37aa21886c0175d65c4e54da1d668db396046` with parent `bd1903af…`. Rebase local observer suite remains 47/47 PASS. After #986 rollout is production-healthy, rebase #987 once more onto `e8fd1942…`, rerun exact-head gates, then merge/install only if normal disk gate remains green.
- Root disk hovered only a few MiB around the 92% hard gate. Preserved all unique/dirty/process-in-use worktrees. Removed only clean merged #980/#983 checkout trees with branch refs intact. Used the existing Hava81 observer logrotate policy twice across a real observer write so delayed compression converted 7.9 MiB `history.jsonl.1` into a ~342 KiB `.2.gz`; no log-retention policy was weakened. Direct disk calculation immediately afterward showed normal gate green; the API-build reserve still remains roughly 0.5 GiB short.
- Do not remove or mutate #986's parallel-source worktree/ref if discovered; its work is now represented by merged main. #981/#982 remain the only prepared API-runtime queue and remain blocked on `api_build_headroom_ok=true`.

### 2026-09-05 13:07 TRT — exact end-of-run checkpoint
- #986 (`baec6906b530b9958dc8d3852e96d315455b4baf`) merged and its main `e8fd19428f61b35b74215f7bdd4324fa63c293a0` completed CI/CD #2409 fully green with exact public root/İstanbul smoke.
- Observer rate-limit fallback fix survived duplicate/concurrent rebuilds. #985/#988 were closed after exact diff inspection showed they accidentally changed `deploy/oracle/observer/observer.py` mode 100755→100644. The mode-preserving branch was rebased with explicit leases and #990 merged from exact head `42018b460cfb00706877a0613ec450d98268aa38`.
- Current stable main is `e840dd490c46a28fe9f229791cb365b6dc841c5e`. CodeQL #1302 (`33959544408`) and CI/CD #2414 (`33959544401`) completed successfully; production root and `/istanbul/` serve exact `e840dd49…`, API readiness/CORS/boot assets are healthy, preferred API remains 4002, OpenWeather circuit is closed, and normal disk gate is green.
- Installed the observer atomically from a clean detached exact-main worktree `/home/ubuntu/hava81-observer-install-e840`. Installer suite passed 51/51; installed `/usr/local/lib/hava81-worker/observer.py` SHA-256 exactly matches the e840 source (`72d3569f2e5a0907fa63ff81e3077f387836dc14fb7c1e64c4d3e9d6b898b86`), mode is 0755, and `hava81-observer.timer` remains enabled+active. Fresh installed observer at `2026-09-05T10:06:38Z` reports `merge_deploy_ready=True`, no blockers, exact e840 main completed/success, production healthy, normal disk gate green, and GitHub state restored after anonymous quota reset.
- Fresh observer also surfaced 38 very old Hava81 Chromium audit processes (`stale_hava81_browser_processes`). They are a warning, not a production incident or merge blocker. The existing browser-temp cleanup helper only removes inactive profile directories and deliberately refuses paths referenced by running processes, so no process was killed in this run. Next run should identify ownership/cmdline provenance and use a bounded Hava81-specific process cleanup mechanism only if available/safe.
- API-build reserve is still red despite normal disk gate: latest fresh state remains roughly 0.49 GiB short. Current prepared API PRs are #989 `f093e22e238b16622f3c47e1bbf7366a0f8f378a` (blank OpenWeather text rejection) and #991 `cacd02585962df08ee5748ebf4687e8a6db88661` (API error responses no-store); both exact-head hosted gates are green. **Do not merge either until fresh `api_build_headroom_ok=true`; then use the validated 4001 canary/rollback → 4002 stable blue-green flow.**
- A separate unpushed observability patch exists at `/home/ubuntu/hava81-status-recovery-mib-run11-1254`, branch `automation/hava81-status-recovery-mib-run11-1254`, old head `55501b8cbb23544cacdb37d11c2da9ef251ac423`: it adds `recovery_mib` so sub-0.01 GiB disk gaps do not misleadingly render as `recovery_gib=0.0`. Local focused tests 4/4 + diff-check pass. Rebase it onto current stable main before push; first check for a concurrent equivalent to avoid duplication.
- Next queue: (1) bounded stale Hava81 Chrome process provenance/cleanup; (2) rebase/push the MiB disk-status patch if not duplicated; (3) keep #989/#991 green and rebase as main moves but hold merge until API-build reserve is green; (4) continue independent UX/accessibility/performance/trust-boundary improvements; (5) preserve MGM MeteoUyarı deferral unless an authorized stable freshness-aware machine-readable source becomes available.

## 2026-09-05 14:13 TRT — keep root geolocation failure explicit
- Continued independently from exact main `8c833523c72fd7468c271ea73e1582c1b8894d68` while #996 validates separately and API-runtime PRs remain blocked by the dedicated build-headroom reserve.
- Audited the root location gate and found a trust bug: after a user explicitly chose “Konumumu Kullan”, any failed geolocation result was immediately cleared and silently replaced with İstanbul weather. That could make a denied/unavailable location look like a successful location match.
...
## 2026-09-05 14:20 TRT — run 11 exact continuity checkpoint
- Merged #994 from exact head `8aeb4eefe60e9965295dc8814cda6c5d08eb0833`, removing the shared generic Chromium `browser-profile` basename from Hava81-specific stale-process matching while retaining explicit `hava81-*`, `h81deploycheck`, `h81meta`, and `h81text` profiles. Observer tests passed 47/47; the installed observer now reports `stale_count=0` without killing or modifying any browser process.
- Merged #993 after lease-safe rebase from exact head `96b6d7607a403dfdd94cbe2f7b138eecd2dba352`; worker status now reports MiB precision for tiny normal disk recovery gaps instead of misleading `0.0 GiB`. Combined observer/status suite passed 51/51 and the exact resulting observer was installed atomically.
- Merged #995 from exact head `8d6044188b30b28d37135ef4f4b23ef95a7123b7`; main `8c833523c72fd7468c271ea73e1582c1b8894d68` completed CI/CD #2425 successfully. Production root and `/istanbul/` served exact 8c8335, root no longer paid an eager API DNS/TLS connection before city choice, while province pages retained API preconnect for their eager weather bootstrap.
- Prepared #996 from exact 8c8335, head `8898a24d4998771717d5546c09bf2b9890dfd2cb`, gating mobile bottom-nav destinations by available weather data. Exact-head CI/CD #2426 and CodeQL #1314 completed successfully; 104 test files / 732 tests passed and the bottom-nav implementation reached 100% statement/branch/function/line coverage. A concurrent autonomous workstream merged that same exact head, producing current main `f58be640a07d2f006593c235a49019469c23b612`; main CI/CD #2430 and CodeQL #1318 are running at this checkpoint.
- Direct production verification before #996 merge showed root + `/istanbul/` exact 8c8335, API readiness/CORS/boot assets healthy, OpenWeather circuit closed, preferred API port 4002. Fresh observer at `2026-09-05T11:15:47.315160Z` reported normal merge/deploy gate green with `3,875,651,584` free bytes, but API-build reserve remains red by `523,418,993` bytes. Therefore API runtime PRs remain HOLD.
- Safely removed five helper-audited clean linked Hava81 worktrees plus two exact-main detached observer install worktrees after verifying no dirty source was present. Their branch refs were preserved; the final cleanup audit reported 0 further eligible clean linked worktrees. No unrelated project, user data, production/rollback Docker image, or dirty/unrepresented checkout was removed.
- API runtime queue remains #989 head `76ff03a66bb11ebb26b4756760a0131adbe87b1d` (reject blank OpenWeather text) and #991 head `08bb5b5ae2b0f25ae4d79110b3448c1ae3a4abf8` (API error responses `no-store`), both exact-head CI/CD + CodeQL green. **Do not merge until fresh `api_build_headroom_ok=true`; then use validated 4001 canary/rollback -> 4002 stable blue-green flow.**
- Parallel work created #997 head `1ffd7eb9693bc7f8936374c893c6e00ae3e1e9fc`, keeping root geolocation denial/unavailability explicit instead of silently substituting İstanbul. Exact-head CI/CD #2427 + CodeQL #1315 are green; rebase it onto the newest production-healthy main before merge.
- Found the same desktop no-city Map dead-destination bug independently. Closed duplicate #998 without deleting its branch after #999 appeared with the identical product fix plus stronger integration and desktop Chromium regression coverage. #999 head `548d96f4080b2af11f4e697d26421433a4b47098` is the canonical candidate; hosted gates are still running and it must be rebased after #996 rollout before merge.
- Current checkpoint worktree `/home/ubuntu/hava81-run11-checkpoint-1420`, branch `automation/hava81-run11-checkpoint-1420`, base/current main `f58be640a07d2f006593c235a49019469c23b612`.
- Next queue: (1) finish #996 main CI/CD #2430 + CodeQL #1318 and direct public smoke exact f58be640; (2) once production healthy, rebase #997 and #999 separately onto current main with explicit remote-head leases, preserve append-only docs, rerun exact-head gates, merge one at a time with fresh Oracle gate checks; (3) keep #989/#991 rebased/green but unmerged until API-build reserve is genuinely green; (4) continue evidence-based first-viewport/mobile/accessibility/performance work while external gates run; (5) retain strict MGM warning deferral unless a stable authorized freshness-aware machine-readable source is verified.

- Follow-up at `2026-09-05T11:21:01.579939Z`: main CI/CD #2430 completed success and fresh observer/public smoke confirmed production exact `f58be640a07d2f006593c235a49019469c23b612`, root + İstanbul HTTP 200, API ready/CORS/boot assets healthy, preferred API 4002, OpenWeather circuit closed, normal merge gate green. Root keeps the API preconnect absent while İstanbul retains it. API-build reserve remains red by `523,996,529` bytes, so #989/#991 stay HOLD.
- #997 was lease-rebased onto exact f58 main and pushed as new exact head `dc5065aff6d2072538611c7e360dd9af6dc651d6`; CodeQL #1320 is green and CI/CD #2432 is currently in browser-flow validation after frontend/API/build/Lighthouse success. Do not merge until that exact head is fully green and a fresh observer recheck remains green.
- The duplicate-close race around desktop Map gating was independently recovered by concurrent PR #1001, exact f58-based head `bd012e4df4a2b3f3eb346a9c89d93f4bb2e698fa`; use #1001 as canonical and do not reopen #998/#999. Its exact-head CI/CD #2433 + CodeQL #1321 are running.
- Updated immediate queue: (1) when #997 exact `dc5065af...` completes green, fresh observer + mergeability + exact-head verify, then merge #997 and smoke new main; (2) while that rollout runs, continue #1001 gates, then rebase #1001 onto new main if #997 lands first and merge only after rerun green; (3) keep API #989/#991 unmerged until `api_build_headroom_ok=true`; (4) checkpoint PR #1000 itself is documentation-only and may be union-rebased/merged after higher-value product PRs without changing runtime behavior.

- Final run-11 checkpoint update: #997 exact head `dc5065aff6d2072538611c7e360dd9af6dc651d6` passed CI/CD #2432 + CodeQL #1320 (including browser/Lighthouse) and was merged by a concurrent autonomous lease-safe action at `2026-09-05T11:24:25Z`, producing new main `9e5062a58baeda38f8a56bc81d512dd0100766ab`. Main CI/CD #2435 + CodeQL #1323 are running; API/frontend/production-build stages are already green, browser/Lighthouse remain pending. Do not treat production as 9e5062 until that main run and exact public revision smoke are green.
- #1001 (`bd012e4df4a2b3f3eb346a9c89d93f4bb2e698fa`) completed its pre-rebase frontend/API/build/Lighthouse/browser gates successfully on f58 base. After main 9e5062 production is healthy, rebase #1001 onto exact 9e5062 using an explicit lease and rerun all exact-head gates before merge.
- Concurrent PR #1002 head `d8d600128f5538ed1575d4cb9fb5585d5001a129` is already based on 9e5062 and removes the otherwise-unused 4.75rem mobile bottom padding when the hidden bottom nav is absent on the empty root. Keep it independent from #1001 and let hosted gates complete; if #1001 lands first, rebase #1002 afterward.
- API container log review over the last ~30 minutes found no 5xx entries; current API readiness remains healthy on 4002. API runtime #989/#991 still remain blocked solely by the separate API-build reserve gate, which was short by `529,075,569` bytes at the latest fresh pre-#997 observer check.

- Runtime-end update at fresh observer `2026-09-05T11:29:25.799991Z`: main CI/CD #2435 completed fully green and production now serves exact `9e5062a58baeda38f8a56bc81d512dd0100766ab` on both root and city shells; API ready/CORS/boot assets are healthy, OpenWeather circuit closed, preferred API 4002, normal merge/deploy gate green, stale Hava81 browser count 0. Free root disk `3,873,157,120` bytes; API-build reserve remains red by `525,913,457` bytes, so #989/#991 remain HOLD.
- Rebased canonical desktop Map PR #1001 from old exact `bd012e4df4a2b3f3eb346a9c89d93f4bb2e698fa` onto exact healthy main 9e5062 in isolated worktree `/home/ubuntu/hava81-rebase-pr1001-run11`; new exact head `d600cea678c1b9977dc9db244cf5b48dc6ee8649`, 4 files / +9, `git diff --check` clean. Force-update used explicit old-head lease `bd012e4d...`; GitHub now reports base 9e5062, head d600cea, mergeable=true. Hosted CI/CodeQL must rerun green before merge.
- New independent concurrent #1003 head `2342962a14fa809047766ac594b387501fe3852b` restores root route state on browser history navigation; CI/CD #2437 + CodeQL #1325 are running. #1002 head `d8d600128f5538ed1575d4cb9fb5585d5001a129` remains independently in CI/CD #2436 with CodeQL #1324 green. Do not mutate either pending branch from another workstream.
- Final priority for next invocation: (1) verify #1001 d600cea exact-head CI/CD+CodeQL and merge with fresh observer if green; (2) smoke resulting main while #1002/#1003 continue independently, then rebase the next non-overlapping green candidate onto newest main with an explicit lease; (3) keep API #989/#991 blocked until `api_build_headroom_ok=true`; (4) clean only helper-audited clean/main-represented worktrees and preserve all dirty/unrepresented work.

## 2026-09-05 15:26 TRT — current continuity after #1005/#1007 and API queue refresh
- #1005 `b7a63e5cc6fdfb549526fb29e1d272ece5180052` merged to main `f3b6823c5effca114d826a3bb993466e184b074d`; main CI/CD #2448 completed green and public root + İstanbul both served that exact frontend revision. Direct local 4002 readiness stayed 200/ready with OpenWeather circuit closed and `Cache-Control: no-store`.
- Rebuilt and merged operational cleanup accounting as #1007 exact head `17c624be6b1e5e3cacbf2174d0344a8d1f94d27d`, producing current main `4ea17e6eb8fedfa28873aad2e6b5a4c12b329656`. Main CI/CD #2452 is still running at this checkpoint; CodeQL #1340 is green. Do not treat 4ea17e as production-confirmed until the main pipeline and exact public revision smoke are green.
- Source-preserving Git compaction reduced the dirty but ref-preserved standalone `/home/chatgpt/hava81-rain-detail-2135/.git` from ~61 MiB to ~52 MiB without deleting working-tree changes or refs. Docker audit found only ~126.5 MiB reclaimable while Hava81 stable/canary/rollback and unrelated active project images remain protected.
- API runtime trust-boundary work was rebuilt on f3b682 into #1008 head `e67e9832c71a02680fbe1d0f9388cc4ae7f53f8d`; API test/build + CodeQL are green, but frontend CI exposed one unrelated scheduler-sensitive metadata assertion. #1010 head `0375641442b0667da6ebaf204bd5e12e402dfb30` is the bounded current-main test fix that waits for the React metadata effect instead of asserting synchronously.
- API error no-store work was rebuilt on f3b682 into #1009 head `37e8d78037f9ccfa715c54dc7ac2e15a63a150e1`; CodeQL is green and CI/CD #2451 is still running. Both #1008/#1009 remain **HOLD for merge/deploy until fresh `api_build_headroom_ok=true`**, then must be rebased onto latest healthy main and use 4001 canary/rollback -> 4002 stable.
- Root disk remains at the normal gate boundary (~92%, ~3.87 GB free) and the separate API-build reserve is still roughly 0.49 GiB short. No unrelated project data, dirty worktree, active container/image, or rollback resource was removed.
- Immediate queue: (1) finish current main #2452 and exact 4ea17e public + local-4002 smoke; (2) finish #1010 exact-head gates and merge only after fresh observer/production verification; (3) rebuild/rebase #1008/#1009 after #1010/main moves but keep them unmerged until API-build reserve is green; (4) preserve this append-only checkpoint and close stale #1000 only after the replacement PR is safely open; (5) continue bounded UX/accessibility/reliability work while external gates run.
