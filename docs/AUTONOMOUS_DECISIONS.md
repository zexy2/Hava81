...
- 2026-08-30 — Open-Meteo parallel forecast arrays must remain structurally aligned with their time axis. Never use the shortest array to silently truncate a malformed provider payload: required series and every provided optional series must have the exact corresponding hourly/daily length, otherwise fail closed before normalization.
