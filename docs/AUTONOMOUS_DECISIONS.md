...
- 2026-08-29 — Persisted decision profiles are untrusted input. Restore only allowlisted activity kinds, supported temperature sensitivity values, and strict 24-hour `HH:mm` clocks; deduplicate/cap restored activities to the UI contract, preserve an intentional empty activity selection, and discard malformed optional clocks rather than letting corrupted local state alter planning semantics or crash activity/commute flows.
