...
- 2026-08-30 — An API traffic switch must validate the readiness contract, not only an HTTP 2xx. Before accepting a target, require JSON `status=ready`, a timezone-aware timestamp no more than 180 seconds old and no more than 60 seconds in the future, and `Cache-Control: no-store`. After switching public traffic, additionally require exact `Access-Control-Allow-Origin: https://hava81.zekiakgul.dev`; otherwise restore the previous Nginx configuration and leave state markers unchanged.
