...
- 2026-08-30 — UV, dust, and pollen maxima are decision/health context and must not be understated because an upstream model array is shorter than its time axis or contains an invalid model time. Require exact parallel-series alignment and parseable GMT/offset timestamps before calculating rolling maxima; malformed timelines fail closed rather than dropping risky slots.
